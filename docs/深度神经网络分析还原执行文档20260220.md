...
## 39. 2026-02-28 Continuation: Stage D(MNIST) + H3 Power Fix

Goal:

- strengthen cross-modal evidence with real vision data
- validate H3 strict gate on a fresh seed block
- test whether `open` cases are caused by weak signal or statistical power

### 39.1 Stage D (MNIST) real-data rerun

Run:

- `tempdata/pipeline_stage_d_multimodal_20260228_mnist_v1.json`

Result:

1. `val_fused_acc=1.0000`
2. `val_retrieval_top1=0.9883`
3. `val_alignment_cos=0.6184`

Decision:

- Stage D evidence upgraded from synthetic-only to real-data backed.

### 39.2 H3 fresh seed block (`max_per_category=64`)

Runs:

- `tempdata/h3_holdout_validation_20260228_v3_seed20260421.json`
- `tempdata/h3_holdout_validation_20260228_v3_seed20260422.json`
- `tempdata/h3_holdout_validation_20260228_v3_seed20260423.json`
- gatecheck: `tempdata/h3_holdout_validation_20260228_v3_gatecheck_seed20260424.json`

Observed:

1. support_models = `2 / 0 / 3`
2. falsify_models = `0 / 0 / 0`
3. seed-block floor = `0` -> strict gate failed (`goal_1_pending`)

Interpretation:

- no falsification appears, but stability floor is broken by one all-open run.

### 39.3 Power-adjusted seed block (`max_per_category=96`)

Runs:

- `tempdata/h3_holdout_validation_20260228_v4_n96_seed20260422.json`
- `tempdata/h3_holdout_validation_20260228_v4_n96_seed20260425.json`
- `tempdata/h3_holdout_validation_20260228_v4_n96_seed20260426.json`
- gatecheck: `tempdata/h3_holdout_validation_20260228_v4_n96_gatecheck_seed20260427.json`

Observed:

1. support_models = `2 / 2 / 2`
2. falsify_models = `0 / 0 / 0`
3. gatecheck:
   - `seed_block_support_min=2`
   - `seed_block_falsify_max=0`
   - `seed_block_pass=true`
   - `current_run_pass=true`
4. layered progression = `goal_3_pending`

Decision:

- H3 strict seed-block gate recovered after balancing statistical power.
- current bottleneck moves to task-family breadth (`goal_3`), not contradiction risk.

### 39.4 Pipeline defaults + stage coupling fix

Code update:

1. `scripts/start_structure_recovery_process.py`
   - Stage D default switched to `dataset=auto`, `mnist_root=tempdata/data`, `total_samples=12000`, `epochs=6`
   - Stage E `goal_3` changed to a stricter joint criterion:
     - task-level summary must pass
     - latest holdout `goal_3` must also pass (if available)

Regression run:

- `tempdata/structure_recovery_pipeline_kickoff_20260228_v4.json`

Observed:

1. Stage D now records `dataset=mnist`
2. Stage E records `goal_3_pending` and `progression=goal_3_pending`

Decision:

- pipeline now reflects the true bottleneck more faithfully and avoids stale optimistic promotion.
