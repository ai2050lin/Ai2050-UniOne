"""测试DeepSeek7B - CPU加载后手动移GPU, 避免safetensors mmap崩溃"""
import sys, time, gc, os
os.environ["TRANSFORMERS_VERBOSITY"] = "error"
os.environ["TOKENIZERS_PARALLELISM"] = "false"
# 禁用hf_transfer加速(可能引起mmap问题)
...
with open(RESULT, "w", encoding="utf-8") as f:
    f.write(f"=== {target} ({time.strftime('%H:%M:%S')}) ===\n")

log_msg(f"cuda: {torch.cuda.is_available()}, {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'N/A'}")
log_msg(f"GPU: {torch.cuda.get_device_properties(0).total_memory/1e9:.1f}GB")
...
try:
    ...
    # 关键: 先CPU加载, 再移到GPU
    log_msg("[2/4] model on CPU first (avoid mmap crash)...")
    ...
    mdl.eval()
    log_msg(f"  CPU loaded: layers={len(mdl.model.layers)}, d={mdl.config.hidden_size}, t={time.time()-t0:.1f}s")

    # 手动移到GPU
    log_msg("[2b] moving to GPU...")
    mdl = mdl.to("cuda")
    log_msg(f"  GPU VRAM={torch.cuda.memory_allocated()/1e9:.2f}GB, t={time.time()-t0:.1f}s")

    log_msg("[3/4] forward pass...")
