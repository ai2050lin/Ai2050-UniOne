"""
Phase CCXI: 5语法+5语义 因果对偶大样本验证
核心改进:
  1. 扩充语义特征到5种: +voice(主动/被动), +person(人称变换), +formality(语域变换)
  2. n=150对, 5语法+5语义, 统计检验5vs5
  3. 6层采样(减少时间), 精简attn/mlp(只测首尾)
  4. 目标: Mann-Whitney U p<0.01
"""
...
PERSON = [
    ...
    ("I want to travel abroad", "She wants to travel abroad"),
    ("I can solve this problem", "He can solve this problem"),
    ("I was surprised by the result", "She was surprised by the result"),
    ("I appreciate your kindness", "He appreciates your kindness"),
    ("I remember the old days", "She remembers the old days"),
    ("I feel confident about tomorrow", "He feels confident about tomorrow"),
    ("I know the correct procedure", "She knows the correct procedure"),
    ("I decided to take the job", "He decided to take the job"),
    ("I received the invitation", "She received the invitation"),
    ("I understand the complexity", "He understands the complexity"),
    ("I like the new approach", "She likes the new approach"),
    ("I completed the training course", "He completed the training course"),
    ("I wrote the initial draft", "She wrote the initial draft"),
    ("I found the solution quickly", "He found the solution quickly"),
    ("I heard the announcement", "She heard the announcement"),
    ("I plan to attend the event", "He plans to attend the event"),
    ("I noticed the small detail", "She noticed the small detail"),
    ("I chose the blue option", "He chose the blue option"),
    ("I built the prototype model", "She built the prototype model"),
    ("I measured the temperature", "He measured the temperature"),
    ("I designed the new system", "She designed the new system"),
    ("I managed the entire team", "He managed the entire team"),
    ("I discovered the hidden path", "She discovered the hidden path"),
    ("I prepared the final report", "He prepared the final report"),
    ("I created the original concept", "She created the original concept"),
    ("I analyzed the market data", "He analyzed the market data"),
    ("I organized the weekly meeting", "She organized the weekly meeting"),
    ("I tested the latest version", "He tested the latest version"),
    ("I reviewed the submitted paper", "She reviewed the submitted paper"),
    ("I presented the key findings", "He presented the key findings"),
    ("I improved the existing method", "She improved the existing method"),
    ("I evaluated the performance", "He evaluated the performance"),
    ("I recorded the experimental data", "She recorded the experimental data"),
    ("I implemented the required changes", "He implemented the required changes"),
    ("I updated the project timeline", "She updated the project timeline"),
    ("I coordinated the team effort", "He coordinated the team effort"),
    ("I verified the calculation results", "She verified the calculation results"),
    ("I published the research article", "He published the research article"),
    ("I developed the training program", "She developed the training program"),
    ("I supervised the construction work", "He supervised the construction work"),
]
...
def main():
    ...
    out_dir = Path(f"results/causal_fiber/{args.model}_ccxi")
    out_dir.mkdir(parents=True, exist_ok=True)
    sys.stdout = Logger(out_dir / 'run.log')

    path = PATHS[args.model]
    print(f"\n{'='*60}")
    print(f"Phase CCXI: {args.model} (n_pairs={args.n_pairs}, 5 syn + 5 sem)")
    print(f"Path: {path}")
    print(f"{'='*60}")
    ...
    semantic_features = {
        ...
        'voice': gen_pairs(VOICE, N),       # 主动→被动
        'person': gen_pairs(PERSON, N),     # 第一→第三人称
        'formality': gen_pairs(FORMALITY, N), # 正式→非正式
    }
    ...
    # ===== S1: Residual全层扫描 =====
    print(f"\n{'='*60}\nS1: Residual全层扫描 (10 features x 6 layers x {N} pairs)\n{'='*60}", flush=True)
    resid_results = {}
    ...
    # ===== S2: Attn vs MLP (首尾层, tense+sentiment) =====
    print(f"\n{'='*60}\nS2: Attn vs MLP\n{'='*60}", flush=True)
    ...
    for component in ['attn', 'mlp']:
        ...
        for layer_idx in comp_layers:
            ...
            for feat_name in ['tense', 'sentiment', 'voice', 'formality']:
    ...
    for layer_idx in comp_layers:
        ln = f'L{layer_idx}'
        contrib[ln] = {}
        for feat in ['tense', 'sentiment', 'voice', 'formality']:
    ...
    # ===== S4: 对偶假设统计检验 (5 vs 5) =====
    print(f"\n{'='*60}\nS4: 对偶假设统计检验 (5 syn vs 5 sem)\n{'='*60}", flush=True)

    syntactic_names = ['tense', 'polarity', 'number', 'negation', 'question']
    semantic_names = ['sentiment', 'semantic_topic', 'voice', 'person', 'formality']
    ...
    print(f"\nDuality Classification (5 syn vs 5 sem):")
