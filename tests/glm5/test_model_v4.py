"""DS7B测试 - 加try/except捕获崩溃原因"""
import sys, time, gc, os, traceback
...
print(f"cuda mem: {torch.cuda.get_device_properties(0).total_mem/1e9:.1f}GB total", flush=True)
...
try:
    ...
    print(f"[2/4] model (low_cpu_mem_usage=True)...", flush=True)
