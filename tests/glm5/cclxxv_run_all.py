"""
CCLXXV Runner: Runs GLM4 and DeepSeek7B sequentially.
Writes log to file from line 1 for robustness.
"""
import sys, os, time

LOG = r"d:\Ai2050\TransformerLens-Project\tests\glm5_temp\cclxxv_run3.txt"
...
with open(LOG, 'w', encoding='utf-8') as f:
    f.write(f"[{time.strftime('%H:%M:%S')}] === CCLXXV Sequential Runner ===\n")
...
def run_model(model_name):
    ...
    try:
        ...
        for li in tl:
            ...
            exp1.append(lr)
            log(f"  L{li}: gate={lr['gate_cos']:+.3f} up={lr['up_cos']:+.3f} cross={lr['cross_cos']:+.3f} Du={lr['W_down_Du_cos']:+.3f} Dg={lr['W_down_Dg_cos']:+.3f}")
        ...
        for li in tl:
            ...
            # u_sep_ratio: ratio of same-cat to diff-cat angle separation
            if scc and dcc:
            ...
            exp2.append(lr)
            log(f"  L{li}: sep={lr['cos_sep']:+.3f} n90={lr['n90_du']} u_sep={lr['u_sep_ratio']:.2f}")
        ...
        for li in ll:
            ...
            log(f"  L{li}: dg_du_cos={mc:.4f} angle={np.degrees(np.arccos(np.clip(mc, -1, 1))):.1f}°")
        ...
        for li in tl:
            ...
            exp4.append(lr)
            log(f"  L{li}: corr={lr['neuron_corr']:.3f} wr={lr['weighted_corr']:.3f} ratio={lr['overlap_ratio']:.2f}x same={lr['same_cat_corr']:.3f} diff={lr['diff_cat_corr']:.3f}")

        # Save all results with qwen3-compatible naming
        out_dir = f"d:\\Ai2050\\TransformerLens-Project\\results\\causal_fiber\\{model_name}_cclxxv"
        ...
        with open(os.path.join(out_dir, "exp3_delta_u_subspace.json"), 'w', encoding='utf-8') as f:
        ...
        if mid1:
            log(f"EXP1 mid: gate={np.mean([r['gate_cos'] for r in mid1]):+.3f} up={np.mean([r['up_cos'] for r in mid1]):+.3f} Du={np.mean([r['W_down_Du_cos'] for r in mid1]):+.3f}")
        mid4 = [r for r in exp4 if n_layers*0.3 <= r["layer"] < n_layers*0.7]
        if mid4:
            log(f"EXP4 mid: corr={np.mean([r['neuron_corr'] for r in mid4]):.3f} ratio={np.mean([r['overlap_ratio'] for r in mid4]):.2f}x")
...
if __name__ == "__main__":
    # Run GLM4 first, then DeepSeek7B
