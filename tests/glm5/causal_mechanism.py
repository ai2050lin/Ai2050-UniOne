"""
Phase CC: 因果机制深度分析 — 全层纤维扭转+500次置换+权重贡献
================================================================
...
核心测试:
  M1: 全层纤维扭转追踪 — 每层的极性∩时态因果cos + 累积旋转角
      大样本: 150对/特征, 10层采样
  M2: 500次置换验证DS7B L27 — 确认p值
  M3: 修复的权重贡献分析 — 正确提取Attn/MLP对旋转的贡献
...
# ============================================================
# M1: 全层纤维扭转追踪 — 每层因果cos + 累积旋转
# ============================================================

def test_m1(model_name, model, tokenizer, device, d_model, n_layers):
    """M1: 全层因果纤维扭转 — 大样本150对/特征"""
    print("\n" + "=" * 70)
    print("M1: 全层因果纤维扭转追踪 (150对/特征)")
    print("=" * 70)
    ...
    if len(sample_layers) > 12:
        step = len(sample_layers) // 10
    ...
    n_half_p = len(pol_aff)
    
    pol_repr = extract_residual_stream(model, tokenizer, device, pol_texts, sample_layers, 'last')
    ...
    tense_repr = extract_residual_stream(model, tokenizer, device, tense_texts, sample_layers, 'last')
    
    # 计算每层的因果方向和交叉余弦
    results = {'sample_layers': sample_layers, 'n_pairs': 150}
    pol_dirs = {}
    tense_dirs = {}
    
    print(f"\n  {'Layer':>6} {'pol_norm':>10} {'tense_norm':>11} {'cross_cos':>10} {'cumul_rot':>11} {'adj_rot':>8}")
    print("  " + "-" * 60)
    ...
    for li in sample_layers:
        # 极性差分方向
        X_pol = pol_repr[li]
        pol_dir = X_pol[n_half_p:].mean(axis=0) - X_pol[:n_half_p].mean(axis=0)
        pol_norm = np.linalg.norm(pol_dir)
        if pol_norm > 1e-10: pol_dir_n = pol_dir / pol_norm
        else: pol_dir_n = np.zeros(d_model)
        pol_dirs[li] = pol_dir_n
        
        # 时态差分方向
        X_tense = tense_repr[li]
        tense_dir = X_tense[n_half_t:].mean(axis=0) - X_tense[:n_half_t].mean(axis=0)
        tense_norm = np.linalg.norm(tense_dir)
        if tense_norm > 1e-10: tense_dir_n = tense_dir / tense_norm
        else: tense_dir_n = np.zeros(d_model)
        tense_dirs[li] = tense_dir_n
        
        # 交叉余弦(极性∩时态)
        cross_cos = float(np.dot(pol_dir_n, tense_dir_n))
        ...
        if li == sample_layers[0]:
            ...
            cos_base = float(np.dot(pol_dir_n, base_pol))
            cumul_rot = float(np.degrees(np.arccos(np.clip(cos_base, -1, 1))))
        
        # 相邻层旋转角
        if prev_pol_dir is not None:
            adj_cos = float(np.dot(pol_dir_n, prev_pol_dir))
            adj_rot = float(np.degrees(np.arccos(np.clip(adj_cos, -1, 1))))
        else:
            adj_rot = 0.0
        
        print(f"  L{li:>4} {pol_norm:>10.2f} {tense_norm:>11.2f} {cross_cos:>+10.4f} {cumul_rot:>10.1f}° {adj_rot:>7.1f}°")
        
        results[li] = {
            'pol_norm': float(pol_norm),
            'tense_norm': float(tense_norm),
            'cross_cos': cross_cos,
            'cumulative_rotation': cumul_rot,
            'adjacent_rotation': adj_rot,
        }
        
        prev_pol_dir = pol_dir_n
        prev_tense_dir = tense_dir_n
    
    # 分析扭转模式
    first_cos = results[sample_layers[0]]['cross_cos']
    last_cos = results[sample_layers[-1]]['cross_cos']
    last_cumul = results[sample_layers[-1]]['cumulative_rotation']
    
    print(f"\n  扭转模式分析:")
    print(f"  首层 cross_cos = {first_cos:+.4f}")
    print(f"  末层 cross_cos = {last_cos:+.4f}")
    print(f"  变化量 = {last_cos - first_cos:+.4f}")
    print(f"  末层累积旋转 = {last_cumul:.1f}°")
    
    if first_cos < -0.1 and last_cos > 0.1:
        print(f"  → ★★★ 纤维扭转: 首层反平行→末层正汇聚! ★★★")
    elif first_cos > 0.1 and last_cos > 0.1:
        print(f"  → 持续正汇聚")
    elif first_cos < -0.1 and last_cos < -0.1:
        print(f"  → 持续反平行")
    else:
    ...
    for li in sample_layers[1:]:
        if results[li]['adjacent_rotation'] > max_adj_rot:
            max_adj_rot = results[li]['adjacent_rotation']
            max_adj_layer = li
    print(f"  最大相邻旋转: L{max_adj_layer} ({max_adj_rot:.1f}°)")
...
def test_m2(model_name, model, tokenizer, device, d_model, n_layers):
    """M2: 500次置换 — 验证因果对齐显著性"""
    print("\n" + "=" * 70)
    print("M2: 500次置换验证因果对齐")
    print("=" * 70)
    ...
    for li in key_layers:
        # 真实因果方向
        X_pol = pol_repr[li]
        pol_dir = X_pol[n_half:].mean(axis=0) - X_pol[:n_half].mean(axis=0)
        pol_norm = np.linalg.norm(pol_dir)
        ...
        p_two = float(np.mean(np.abs(perm_cos_values) >= abs(real_cos)))
        p_one = float(np.mean(perm_cos_values >= real_cos)) if real_cos > 0 else float(np.mean(perm_cos_values <= real_cos))
        z_score = float((real_cos - np.mean(perm_cos_values)) / max(np.std(perm_cos_values), 1e-6))
        
        sig = " ★★★" if p_two < 0.01 else (" ★★" if p_two < 0.05 else (" ★" if p_two < 0.10 else ""))
        
        print(f"\n  L{li}: real={real_cos:+.4f}")
        print(f"    perm(500): mean={np.mean(perm_cos_values):+.4f}, std={np.std(perm_cos_values):.4f}")
        print(f"    |cos|: 95th={np.percentile(np.abs(perm_cos_values), 95):.4f}, 99th={np.percentile(np.abs(perm_cos_values), 99):.4f}")
        print(f"    p(two)={p_two:.4f}, p(one)={p_one:.4f}, z={z_score:.2f}{sig}")
        
        results[li] = {
            'real_cos': real_cos,
            'perm_mean': float(np.mean(perm_cos_values)),
            'perm_std': float(np.std(perm_cos_values)),
            'perm_p95': float(np.percentile(np.abs(perm_cos_values), 95)),
            'perm_p99': float(np.percentile(np.abs(perm_cos_values), 99)),
            'p_two_sided': p_two,
            'p_one_sided': p_one,
            'z_score': z_score,
        }
...
def test_m3(model_name, model, tokenizer, device, d_model, n_layers):
    """M3: 修复的Attn/MLP贡献分析"""
    print("\n" + "=" * 70)
    print("M3: Attn/MLP对最后一层旋转的贡献")
    print("=" * 70)
    ...
    print(f"  旋转角: {rotation_angle:.1f}°")
    print(f"  差分范数: prev={prev_norm:.2f}, last={last_norm:.2f}")
    ...
    # 切向/法向分解
    proj_tang = np.dot(delta, prev_diff_n) * prev_diff_n
    proj_norm_vec = delta - proj_tang
    tang_mag = np.linalg.norm(proj_tang)
    norm_mag = np.linalg.norm(proj_norm_vec)
    
    print(f"  变化向量范数: {delta_norm:.4f}")
    print(f"  切向: {tang_mag:.4f} ({tang_mag/max(delta_norm,1e-6)*100:.1f}%)")
    print(f"  法向: {norm_mag:.4f} ({norm_mag/max(delta_norm,1e-6)*100:.1f}%)")
    
    # 权重分析 — 用hook方法直接捕获Attn/MLP输出
