...
# -*- coding: utf-8 -*-
"""
Stage640: 差异方向注入恢复实验——语法/relation/coref三类能力的delta_l因果验证

目标：
1. 在 syntax / relation / coref 三类任务上找到"最伤"的组件消融点
2. 在同一位置注入 clean 差异方向，测试能否恢复 margin
3. 四模型比较：差异方向是否具有恢复能力，恢复更依赖 attn 还是 mlp

预注册判伪条件：
  如果所有6个case中recovery_ratio均值<0.3（恢复不到30%），则
  "差异方向具有因果恢复力"(INV-186)被推翻。

用法：
  python stage640_direction_injection_recovery.py [qwen3|deepseek7b|glm4|gemma4]
"""
...
def main() -> None:
    if len(sys.argv) < 2:
        print("用法: python stage640_direction_injection_recovery.py [qwen3|deepseek7b|glm4|gemma4]")
        sys.exit(1)
    model_key = sys.argv[1]
    run_dir = OUTPUT_DIR / f"stage640_direction_injection_recovery_{model_key}_{TIMESTAMP}"
    run_dir.mkdir(parents=True, exist_ok=True)

    model = None
    try:
        print(f"[Stage640] 加载模型 {model_key}...")
        model, tokenizer = load_model_bundle(model_key, prefer_cuda=True)
        layer_indices = evenly_spaced_layers(model, count=5)
        print(f"[Stage640] 层索引: {layer_indices}")
        records: List[Dict[str, object]] = []

        for i, case in enumerate(CASES):
            print(f"[Stage640] ({i+1}/{len(CASES)}) {case.capability}/{case.pair_id}")
        ...
        falsified = overall_recovery < 0.3
        print(f"\n[Stage640] 整体recovery_ratio={overall_recovery:.4f}")
        print(f"[Stage640] 预注册判伪: {'INV-186被推翻' if falsified else 'INV-186存活'}")
