...
def run_model(model_name):
    ...
    try:
        ...
        for li in tl:
            ...
            exp1.append(lr)
            log(f"  L{li}: gate={lr['gate_cos']:+.3f} up={lr['up_cos']:+.3f} Du={lr['W_down_Du_cos']:+.3f} Dg={lr['W_down_Dg_cos']:+.3f}")
        ...
        for li in tl:
            ...
            lr = {"layer": li, "same_cos": float(np.mean(scc)) if scc else 0,
            ...
            exp2.append(lr)
            log(f"  L{li}: sep={lr['cos_sep']:+.3f} n90={lr['n90_du']}")
        ...
        for li in ll:
            ...
            same_l.append({"layer": li, "dg_du_cos": float(np.mean(ca))})
            log(f"  L{li}: dg_du_cos={float(np.mean(ca)):.3f}")
        ...
        for li in tl:
            ...
            exp4.append(lr)
            log(f"  L{li}: corr={lr['neuron_corr']:.3f} wr={lr['weighted_corr']:.3f} ratio={lr['overlap_ratio']:.2f}x")
        ...
        def js(obj):
            ...
            return obj
        for name, data in [("exp1", exp1), ("exp2", exp2), ("exp4", exp4)]:
        ...
        with open(os.path.join(out_dir, "exp3.json"), 'w', encoding='utf-8') as f:
            json.dump(js({"experiment": "exp3", "model": model_name, "du_cross_layer": du_rot, "dg_du_same_layer": same_l}), f, indent=2)
        log(f"Saved to {out_dir}")

        # Summary
        mid1 = [r for r in exp1 if n_layers*0.3 <= r["layer"] < n_layers*0.7]
        if mid1:
            log(f"EXP1 mid: gate_cos={np.mean([r['gate_cos'] for r in mid1]):+.3f} up_cos={np.mean([r['up_cos'] for r in mid1]):+.3f} Du={np.mean([r['W_down_Du_cos'] for r in mid1]):+.3f}")
...
if __name__ == "__main__":
    ...
    with open(LOG, 'w', encoding='utf-8') as f:
        f.write(f"[{time.strftime('%H:%M:%S')}] === NEW RUN: {args.model} ===\n")
