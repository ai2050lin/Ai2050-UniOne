...
class LawConditionedQueryReadoutGrounder(c44.ManifoldConditionedConsistencyHeadGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        query_family_weight: float,
        query_vote_weight: float,
        query_identity_gain: float,
        query_memory_gain: float,
        query_stabilize_gain: float,
        ...
        self.query_family_weight = query_family_weight
        self.query_vote_weight = query_vote_weight
        self.query_identity_gain = query_identity_gain
        self.query_memory_gain = query_memory_gain
        self.query_stabilize_gain = query_stabilize_gain
        self.selective_penalty_weight = selective_penalty_weight

    def _law_gain(self, concept: str) -> float:
        rule = self.rule_state.get(concept)
        if rule == "identity":
            return self.query_identity_gain
        if rule == "memory":
            return self.query_memory_gain
        if rule == "stabilize":
            return self.query_stabilize_gain
        return 0.8 * self.query_identity_gain
    ...
    def _query_gate(self, x: np.ndarray, family: str, concept: str) -> Tuple[float, np.ndarray | None]:
        ...
        if family_state is not None:
            readout_energy += self.query_family_weight * c7.sq_dist(readout, family_state)
        if vote is not None:
            readout_energy += self.query_vote_weight * c7.sq_dist(x, vote)
        gain = self._law_gain(concept)
        delta = (manifold_energy - gain * readout_energy) / max(1e-4, self.query_temperature)
        gate = 1.0 / (1.0 + np.exp(-delta))
        target = ((1.0 - gate) * manifold + gate * readout).astype(np.float32)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = LawConditionedQueryReadoutGrounder(
        ...
        query_family_weight=float(config["query_family_weight"]),
        query_vote_weight=float(config["query_vote_weight"]),
        query_identity_gain=float(config["query_identity_gain"]),
        query_memory_gain=float(config["query_memory_gain"]),
        query_stabilize_gain=float(config["query_stabilize_gain"]),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C47 law-conditioned query readout search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c47_law_conditioned_query_readout_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for query_temperature in [0.16, 0.28]:
        for query_family_weight in [0.08]:
            for query_vote_weight in [0.12]:
                for query_identity_gain in [1.0, 1.15]:
                    for query_memory_gain in [0.75, 0.90]:
                        for query_stabilize_gain in [1.0, 1.10]:
                            for selective_penalty_weight in [0.02]:
                                config = {
                                    **base,
                                    "query_temperature": query_temperature,
                                    "query_family_weight": query_family_weight,
                                    "query_vote_weight": query_vote_weight,
                                    "query_identity_gain": query_identity_gain,
                                    "query_memory_gain": query_memory_gain,
                                    "query_stabilize_gain": query_stabilize_gain,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c47_law_conditioned_query_not_enough"
        core_answer = "A law-conditioned query improves controller-aware readout, but still does not produce a better retention-compatible family."
        main_open_gap = "law_conditioned_query_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c47_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c47_retention_compatible_law_conditioned_query_found"
        )
        core_answer = (
            "A law-conditioned query creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A law-conditioned query reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c47"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC47_law_conditioned_query_readout_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c47_beats_c42_retention_compatible": bool(
            ...
            "H2_c47_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
