...
import test_stage_c40_law_conditioned_latent_search as c40
...
class StrongCrossLawCoordinateGrounder(c40.LawConditionedLatentGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.manifold_coordinate_pull = manifold_coordinate_pull
        self.manifold_identity_weight = manifold_identity_weight
        self.manifold_persistence_weight = manifold_persistence_weight
        self.manifold_penalty_weight = manifold_penalty_weight
        self.shared_manifold: Dict[str, np.ndarray] = {}

    def _update_shared_manifold(self, concept: str) -> None:
        rows = []
        latent = self.shared_latent.get(concept)
        identity = self.identity_preserve_map.get(concept)
        persist = self.persistence_state.get(concept)
        if latent is not None:
            rows.append(latent.astype(np.float32))
        if identity is not None:
            rows.append(self.manifold_identity_weight * identity.astype(np.float32))
        if persist is not None:
            rows.append(self.manifold_persistence_weight * persist.astype(np.float32))
        if not rows:
            return
        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.shared_manifold.get(concept)
        self.shared_manifold[concept] = (
            target
            if prev is None
            else ((1.0 - self.manifold_coordinate_pull) * prev + self.manifold_coordinate_pull * target).astype(np.float32)
        )

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        self._update_shared_manifold(concept)
        ...
        if rule in {"memory", "stabilize"} and identity is not None:
            self.identity_path[concept] = ((1.0 - 0.14) * identity + 0.14 * manifold).astype(np.float32)
        if rule in {"identity", "stabilize"} and memory is not None:
            self.memory_path[concept] = ((1.0 - 0.08) * memory + 0.08 * manifold).astype(np.float32)
        if novelty is not None and rule == "identity":
            self.novelty_path[concept] = ((1.0 - 0.06) * novelty + 0.06 * manifold).astype(np.float32)
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if manifold is None:
            return base
        return float(base + self.manifold_penalty_weight * c7.sq_dist(x, manifold))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = StrongCrossLawCoordinateGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C41 strong cross-law coordinate search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c41_strong_cross_law_coordinate_search_20260312.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c40 = json.loads(
        (TEMP_DIR / "stage_c40_law_conditioned_latent_search_20260311.json").read_text(encoding="utf-8")
    )
    stage_c15 = json.loads(
        (TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8")
    )
    base = dict(stage_c40["best_objective_candidate"]["config"])
    ...
    for manifold_coordinate_pull in [0.10, 0.22]:
        for manifold_identity_weight in [0.40, 0.70]:
            for manifold_persistence_weight in [0.30, 0.55]:
                for manifold_penalty_weight in [0.02, 0.06]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c41_strong_cross_law_coordinate_not_enough"
        core_answer = "A stronger cross-law manifold raises shared coordination, but still does not produce a better retention-compatible family."
        main_open_gap = "strong_cross_law_manifold_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c41_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c41_retention_compatible_strong_cross_law_coordinate_found"
        )
        core_answer = (
            "A stronger cross-law manifold creates a retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A stronger cross-law manifold reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c41"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC41_strong_cross_law_coordinate_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c41_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c41_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
