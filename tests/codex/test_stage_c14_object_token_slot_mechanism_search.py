...
class ObjectTokenSlotGrounder(c7.ConsensusDiscriminatorTemporalGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.object_token_weight = object_token_weight
        self.write_gate = write_gate
        self.read_pull = read_pull
        self.persistence_pull = persistence_pull
        self.write_slots: Dict[str, np.ndarray] = {}
        self.read_slots: Dict[str, np.ndarray] = {}
        self.identity_persistence: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        prev_write = self.write_slots.get(concept)
        if prev_write is None:
            self.write_slots[concept] = full_proto.copy()
        else:
            self.write_slots[concept] = (
                (1.0 - self.write_gate) * prev_write + self.write_gate * full_proto
            ).astype(np.float32)

        prev_read = self.read_slots.get(concept)
        if prev_read is None:
            self.read_slots[concept] = self.write_slots[concept].copy()
        else:
            self.read_slots[concept] = (
                (1.0 - self.read_pull) * prev_read + self.read_pull * self.write_slots[concept]
            ).astype(np.float32)

        prev_persist = self.identity_persistence.get(concept)
        if prev_persist is None:
            self.identity_persistence[concept] = self.read_slots[concept].copy()
        else:
            self.identity_persistence[concept] = (
                (1.0 - self.persistence_pull) * prev_persist + self.persistence_pull * self.read_slots[concept]
            ).astype(np.float32)
    ...
    def _concept_score(self, x: np.ndarray, observed: List[Tuple[str, np.ndarray]], family: str, concept: str, family_score: float) -> float:
        ...
        read_slot = self.read_slots.get(concept)
        persist = self.identity_persistence.get(concept)
        if read_slot is not None:
            score += self.object_token_weight * c7.sq_dist(x, read_slot)
        if persist is not None:
            score += 0.5 * self.object_token_weight * c7.sq_dist(x, persist)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = ObjectTokenSlotGrounder(
        ...
        object_token_weight=float(config["object_token_weight"]),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C14 object-token slot mechanism search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c14_object_token_slot_mechanism_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    base = dict(stage_c8["best_retention_compatible_candidate"]["config"])
    ...
    for object_token_weight in [0.1, 0.25]:
        for write_gate in [0.25, 0.5, 0.75]:
            for read_pull in [0.05, 0.15, 0.3]:
                for persistence_pull in [0.02, 0.08]:
                    config = {
                        **base,
                        "object_token_weight": object_token_weight,
    ...
    if best_retention_compatible is None:
        status = "stage_c14_new_mechanism_class_not_enough"
        core_answer = (
            "Explicit object-token slots improve the family locally, but still do not create a stronger retention-compatible closure."
        )
        main_open_gap = "needs_mechanism_beyond_object_token_slots"
        ...
        status = (
            "stage_c14_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c14_retention_compatible_family_strengthened"
        )
        core_answer = (
            "Explicit object-token slots strengthen the retention-compatible family, but still do not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "An object-token slot family reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c14"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC14_object_token_slot_mechanism_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c14_improves_retention_compatible_consistency": bool(
            ...
            "H2_c14_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
