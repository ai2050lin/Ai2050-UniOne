...
import test_stage_c41_strong_cross_law_coordinate_search as c41
...
class StrongCrossLawManifoldLiftGrounder(c41.StrongCrossLawCoordinateGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.family_manifold_pull = family_manifold_pull
        self.family_manifold_weight = family_manifold_weight
        self.relation_pull = relation_pull
        self.temporal_vote_pull = temporal_vote_pull
        self.family_manifold: Dict[str, np.ndarray] = {}
        self.temporal_vote_state: Dict[str, np.ndarray] = {}

    def _update_family_manifold(self, family: str) -> None:
        rows = []
        for concept, concept_family in self.concept_family.items():
        ...
        prev = self.family_manifold.get(family)
        self.family_manifold[family] = (
            target
            if prev is None
            else ((1.0 - self.family_manifold_pull) * prev + self.family_manifold_pull * target).astype(np.float32)
        )

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        self._update_family_manifold(family)
        manifold = self.shared_manifold.get(concept)
        ...
        if family_state is not None:
            self.shared_manifold[concept] = (
                (1.0 - self.family_manifold_weight) * manifold + self.family_manifold_weight * family_state
            ).astype(np.float32)
        prev_vote = self.temporal_vote_state.get(concept)
        self.temporal_vote_state[concept] = (
            self.shared_manifold[concept]
            if prev_vote is None
            else ((1.0 - self.temporal_vote_pull) * prev_vote + self.temporal_vote_pull * self.shared_manifold[concept]).astype(np.float32)
        )
        vote = self.temporal_vote_state.get(concept)
        identity = self.identity_path.get(concept)
        if vote is not None and identity is not None:
            self.identity_path[concept] = (
                (1.0 - self.relation_pull) * identity + self.relation_pull * vote
            ).astype(np.float32)
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        family_state = None if family is None else self.family_manifold.get(family)
        vote = self.temporal_vote_state.get(concept)
        penalty = base
        if family_state is not None:
            penalty += 0.5 * self.family_manifold_weight * c7.sq_dist(x, family_state)
        if vote is not None:
            penalty += 0.5 * self.temporal_vote_pull * c7.sq_dist(x, vote)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = StrongCrossLawManifoldLiftGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C42 strong cross-law manifold lift search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c42_strong_cross_law_manifold_lift_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for family_manifold_pull in [0.08, 0.18]:
        for family_manifold_weight in [0.06, 0.14]:
            for relation_pull in [0.05, 0.12]:
                for temporal_vote_pull in [0.08, 0.18]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c42_strong_cross_law_manifold_not_enough"
        core_answer = "A lifted cross-law manifold improves shared structure, but still does not produce a better retention-compatible family."
        main_open_gap = "cross_law_manifold_lift_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c42_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c42_retention_compatible_cross_law_manifold_lift_found"
        )
        core_answer = (
            "A lifted cross-law manifold creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A lifted cross-law manifold reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c42"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC42_strong_cross_law_manifold_lift_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c42_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c42_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
