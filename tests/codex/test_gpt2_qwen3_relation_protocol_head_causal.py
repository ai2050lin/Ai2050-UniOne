#!/usr/bin/env python
"""
Small-scale causal validation for relation protocol heads in GPT-2 and Qwen3.

For each relation family:
1) take the best TT-carrying head found by the head atlas
2) ablate that head
3) compare against a matched-layer control head

We measure collapse of the relation family's model-level TT bridge:
    bridge_tt(l) = endpoint_topo_basis(l) * relation_align_topo(l)
"""
...
def pick_control_head(model_head_atlas: Dict[str, object], relation_name: str, best_layer: int, n_heads: int) -> Tuple[int, int]:
    top_set = {
        (int(row["layer"]), int(row["head"]))
        for row in model_head_atlas["relations"][relation_name]["top_heads"]
    }
    for hi in range(n_heads):
        if (best_layer, hi) not in top_set:
            return best_layer, hi
    return best_layer, int((model_head_atlas["relations"][relation_name]["summary"]["best_head"]["head"] + 1) % n_heads)


def compute_relation_bridge(model, tok, ablator: HeadAblator | None, relation_name: str, condition: Tuple[int, int] | None) -> Dict[str, object]:
    if ablator is not None:
        if condition is None:
            ablator.clear()
        else:
            ablator.set_active(condition[0], condition[1])
...
def analyze_model(model_name: str, model_path: str, dtype_name: str, prefer_cuda: bool) -> Dict[str, object]:
    ...
    try:
        for relation_name in relation_specs().keys():
            best = head_atlas["relations"][relation_name]["summary"]["best_head"]
            best_head = (int(best["layer"]), int(best["head"]))
            control_head = pick_control_head(head_atlas, relation_name, best_head[0], n_heads)

            baseline = compute_relation_bridge(model, tok, ablator, relation_name, None)
            top_ablate = compute_relation_bridge(model, tok, ablator, relation_name, best_head)
            ctrl_ablate = compute_relation_bridge(model, tok, ablator, relation_name, control_head)
            ...
            relations[relation_name] = {
                "best_head": {"layer": best_head[0], "head": best_head[1]},
                "control_head": {"layer": control_head[0], "head": control_head[1]},
                "baseline": baseline,
                "top_head_ablation": top_ablate,
                "control_ablation": ctrl_ablate,
                "summary": {
                    "baseline_peak_bridge_tt": base_peak,
                    "top_ablation_peak_bridge_tt": top_peak,
                    "control_ablation_peak_bridge_tt": ctrl_peak,
                    "top_collapse_ratio": float(max(0.0, (base_peak - top_peak) / (base_peak + eps))),
                    "control_collapse_ratio": float(max(0.0, (base_peak - ctrl_peak) / (base_peak + eps))),
                    "causal_margin": float((ctrl_peak - top_peak) / (base_peak + eps)),
                    "baseline_peak_layer": int(baseline["peak_layer"]),
                    "top_ablation_peak_layer": int(top_ablate["peak_layer"]),
                    "control_ablation_peak_layer": int(ctrl_ablate["peak_layer"]),
                },
            }
    finally:
        ablator.close()
    ...
    collapse_top = [row["summary"]["top_collapse_ratio"] for row in relations.values()]
    collapse_ctrl = [row["summary"]["control_collapse_ratio"] for row in relations.values()]
    causal_margin = [row["summary"]["causal_margin"] for row in relations.values()]
    ...
    return {
        ...
        "relations": relations,
        "global_summary": {
            "mean_top_collapse_ratio": float(np.mean(collapse_top)),
            "mean_control_collapse_ratio": float(np.mean(collapse_ctrl)),
            "mean_causal_margin": float(np.mean(causal_margin)),
            "stronger_than_control_count": int(sum(1 for a, b in zip(collapse_top, collapse_ctrl) if a > b)),
        },
    }


def main() -> None:
    ap = argparse.ArgumentParser(description="Causal validation for relation protocol heads in GPT-2 and Qwen3")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/gpt2_qwen3_relation_protocol_head_causal_20260308.json",
    )
    args = ap.parse_args()
