...
class StrongerIdentityBridgeGrounder(c30.ThreePathArchitectureGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        bridge_pull: float,
        bridge_identity_weight: float,
        bridge_memory_weight: float,
        bridge_gate_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.bridge_identity_weight = bridge_identity_weight
        self.bridge_memory_weight = bridge_memory_weight
        self.bridge_gate_weight = bridge_gate_weight
        self.identity_bridge: Dict[str, np.ndarray] = {}

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        id_state = self.identity_path.get(concept)
        mem_state = self.memory_path.get(concept)
        nov_state = self.novelty_path.get(concept)
        if id_state is None or mem_state is None:
            return
        target = (
            self.bridge_identity_weight * id_state
            + self.bridge_memory_weight * mem_state
        )
        if nov_state is not None:
            target = (0.85 * target + 0.15 * nov_state).astype(np.float32)
        prev = self.identity_bridge.get(concept)
        self.identity_bridge[concept] = (
            target.astype(np.float32)
            if prev is None
            else ((1.0 - self.bridge_pull) * prev + self.bridge_pull * target).astype(np.float32)
        )

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        base = super()._fusion_penalty(x, concept)
        bridge = self.identity_bridge.get(concept)
        id_state = self.identity_path.get(concept)
        mem_state = self.memory_path.get(concept)
        if bridge is None or id_state is None or mem_state is None:
            return base
        id_e = c7.sq_dist(x, id_state)
        mem_e = c7.sq_dist(x, mem_state)
        gate = sigmoid(self.bridge_gate_weight * (mem_e - id_e))
        bridge_e = c7.sq_dist(x, bridge)
        # High identity confidence opens the bridge; memory mismatch keeps it bounded.
        bridge_bonus = gate * bridge_e + (1.0 - gate) * 0.25 * bridge_e
        return float(base + bridge_bonus)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = StrongerIdentityBridgeGrounder(
        ...
        bridge_identity_weight=float(config["bridge_identity_weight"]),
        bridge_memory_weight=float(config["bridge_memory_weight"]),
        bridge_gate_weight=float(config["bridge_gate_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C31 three-path + stronger identity bridge search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c31_three_path_stronger_identity_bridge_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    for bridge_pull in [0.08, 0.18]:
        for bridge_identity_weight in [0.60, 0.75]:
            for bridge_memory_weight in [0.25, 0.40]:
                for bridge_gate_weight in [2.0, 4.0]:
                    config = {
                        **base,
                        "bridge_pull": bridge_pull,
                        "bridge_identity_weight": bridge_identity_weight,
                        "bridge_memory_weight": bridge_memory_weight,
                        "bridge_gate_weight": bridge_gate_weight,
                    }
                    rows = []
    ...
    if best_retention_compatible is None:
        status = "stage_c31_bridge_not_enough"
        core_answer = "A stronger identity bridge changes the three-path tradeoff, but still does not produce a better retention-compatible family."
        main_open_gap = "stronger_identity_bridge_still_not_enough"
        ...
        status = (
            "stage_c31_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c31_retention_compatible_bridge_family_found"
        )
        core_answer = (
            "A stronger identity bridge improves the three-path architecture while keeping retention-compatible behavior, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A stronger identity bridge reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c31"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC31_three_path_stronger_identity_bridge_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c31_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c31_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
