...
def split_subspaces(x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return x[identity_idx].astype(np.float32), x[concept_idx].astype(np.float32)


class FactorizedRepresentationGrounder(c24.TwoStageIdentityConceptGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        stage1_topk: int,
        factor_identity_weight: float,
        factor_concept_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.factor_identity_weight = factor_identity_weight
        self.factor_concept_weight = factor_concept_weight
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        x_id, x_con = split_subspaces(x)
        prev_id = self.identity_factor_state.get(concept)
        if prev_id is None:
            self.identity_factor_state[concept] = x_id
        else:
            self.identity_factor_state[concept] = (0.85 * prev_id + 0.15 * x_id).astype(np.float32)
        prev_con = self.concept_factor_state.get(concept)
        if prev_con is None:
            self.concept_factor_state[concept] = x_con
        else:
            self.concept_factor_state[concept] = (0.92 * prev_con + 0.08 * x_con).astype(np.float32)

    def _identity_cluster_score(self, x: np.ndarray, concept: str) -> float:
        base = super()._identity_cluster_score(x, concept)
        x_id, _ = split_subspaces(x)
        id_state = self.identity_factor_state.get(concept)
        if id_state is not None:
            base += self.factor_identity_weight * c7.sq_dist(x_id, id_state)
        return float(base)

    def _concept_memory_score(self, x: np.ndarray, concept: str) -> float:
        base = super()._concept_memory_score(x, concept)
        _, x_con = split_subspaces(x)
        con_state = self.concept_factor_state.get(concept)
        if con_state is not None:
            base += self.factor_concept_weight * c7.sq_dist(x_con, con_state)
        return float(base)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = FactorizedRepresentationGrounder(
        ...
        factor_identity_weight=float(config["factor_identity_weight"]),
        factor_concept_weight=float(config["factor_concept_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C25 factorized representation search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c25_factorized_representation_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    for factor_identity_weight in [0.05, 0.12]:
        for factor_concept_weight in [0.03, 0.08]:
            config = {
                **base,
                "factor_identity_weight": factor_identity_weight,
                "factor_concept_weight": factor_concept_weight,
            }
            rows = []
    ...
    if best_retention_compatible is None:
        status = "stage_c25_factorization_not_enough"
        core_answer = "Factorized identity/concept subspaces soften geometric interference, but still do not create a better retention-compatible family."
        main_open_gap = "factorized_geometry_still_not_enough"
        ...
        status = (
            "stage_c25_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c25_retention_compatible_factorized_family_found"
        )
        core_answer = (
            "Factorized identity/concept subspaces create a new retention-compatible family, but still do not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "Factorized identity/concept subspaces reach strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c25"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC25_factorized_representation_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c25_improves_over_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c25_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
