...
class StrongConsistencyHeadGrounder(c42.StrongCrossLawManifoldLiftGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        head_pull: float,
        head_anchor_weight: float,
        head_same_weight: float,
        head_negative_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.head_pull = head_pull
        self.head_anchor_weight = head_anchor_weight
        self.head_same_weight = head_same_weight
        self.head_negative_weight = head_negative_weight
        self.consistency_head: Dict[str, np.ndarray] = {}

    def _head_target(self, concept: str) -> np.ndarray | None:
        rows = []
        ...
        if manifold is not None:
            rows.append(0.35 * manifold.astype(np.float32))
        if pair is not None:
            rows.append(0.30 * pair.astype(np.float32))
        if preserve is not None:
            rows.append(0.20 * preserve.astype(np.float32))
        if vote is not None:
            rows.append(0.15 * vote.astype(np.float32))
        if not rows:
            return None
        return np.sum(np.stack(rows, axis=0), axis=0).astype(np.float32)

    def _nearest_negative_energy(self, concept: str) -> float:
        family = self.concept_family.get(concept)
        head = self.consistency_head.get(concept)
        ...
        for other, other_family in self.concept_family.items():
            if other == concept or other_family != family:
                continue
            other_head = self.consistency_head.get(other)
            if other_head is None:
                continue
            negatives.append(c7.sq_dist(head, other_head))
        if not negatives:
            return 0.0
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        target = self._head_target(concept)
        if target is None:
            return
        prev = self.consistency_head.get(concept)
        self.consistency_head[concept] = (
            target
            if prev is None
            else ((1.0 - self.head_pull) * prev + self.head_pull * target).astype(np.float32)
        )
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if head is None:
            return penalty
        penalty += self.head_anchor_weight * c7.sq_dist(x, head)
        penalty += self.head_same_weight * (
            0.5 * c7.sq_dist(x, self.shared_manifold.get(concept) if self.shared_manifold.get(concept) is not None else head)
            + 0.5 * c7.sq_dist(x, head)
        )
        penalty += self.head_negative_weight * self._nearest_negative_energy(concept)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = StrongConsistencyHeadGrounder(
        ...
        head_pull=float(config["head_pull"]),
        head_anchor_weight=float(config["head_anchor_weight"]),
        head_same_weight=float(config["head_same_weight"]),
        head_negative_weight=float(config["head_negative_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C43 strong consistency head search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c43_strong_consistency_head_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    base = dict(stage_c42["best_objective_candidate"]["config"])
    ...
    for head_pull in [0.10, 0.22]:
        for head_anchor_weight in [0.03, 0.08]:
            for head_same_weight in [0.04, 0.10]:
                for head_negative_weight in [0.03, 0.08]:
                    config = {
                        **base,
                        "head_pull": head_pull,
                        "head_anchor_weight": head_anchor_weight,
                        "head_same_weight": head_same_weight,
                        "head_negative_weight": head_negative_weight,
                    }
                    rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c43_strong_consistency_head_not_enough"
        core_answer = "A stronger consistency head sharpens discrimination, but still does not produce a better retention-compatible family."
        main_open_gap = "consistency_head_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c43_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c43_retention_compatible_strong_consistency_head_found"
        )
        core_answer = (
            "A stronger consistency head creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A stronger consistency head reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c43"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC43_strong_consistency_head_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c43_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c43_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
