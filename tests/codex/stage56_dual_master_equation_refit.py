...
def build_rows(rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        ...
        strict_module = safe_float(row.get("strict_load_term")) + safe_float(row.get("logic_strictload_term"))
        ...
        out.append(
            {
                **dict(row),
                ...
                "strict_module_term": strict_module,
                "dual_gap_term": dual_gap,
            }
        )
    return out
...
def build_summary(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        "record_type": "stage56_dual_master_equation_refit_summary",
        "row_count": len(rows),
        "feature_names": names,
        "fits": fits,
        ...
        "main_judgment": (
            "双主式已经可以写成一般闭包核、严格闭包模块和二者的边距三层，"
            "当前最关键的问题是一般核与严格模块是否已经足够分离到能独立成式。"
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Refit a dual master equation with a general kernel and a strict module")
    ap.add_argument(
        "--input-rows-json",
    ...
    ap.add_argument(
        "--output-dir",
        default=str(ROOT / "tests" / "codex_temp" / "stage56_dual_master_equation_refit_20260319"),
    )
