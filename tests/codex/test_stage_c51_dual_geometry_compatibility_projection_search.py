...
class DualGeometryCompatibilityGrounder(c44.ManifoldConditionedConsistencyHeadGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        projection_pull: float,
        manifold_projection_weight: float,
        readout_projection_weight: float,
        family_projection_weight: float,
        identity_projection_weight: float,
        ...
        self.projection_pull = projection_pull
        self.manifold_projection_weight = manifold_projection_weight
        self.readout_projection_weight = readout_projection_weight
        self.family_projection_weight = family_projection_weight
        self.identity_projection_weight = identity_projection_weight
        self.projection_penalty_weight = projection_penalty_weight
        self.compatibility_projection: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if manifold is not None:
            rows.append(self.manifold_projection_weight * manifold.astype(np.float32))
        if readout is not None:
            rows.append(self.readout_projection_weight * readout.astype(np.float32))
        if family_state is not None:
            rows.append(self.family_projection_weight * family_state.astype(np.float32))
        if identity is not None:
            rows.append(self.identity_projection_weight * identity.astype(np.float32))
        if not rows:
            return
        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.compatibility_projection.get(concept)
        self.compatibility_projection[concept] = (
            target
            if prev is None
            else ((1.0 - self.projection_pull) * prev + self.projection_pull * target).astype(np.float32)
        )

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        penalty = super()._fusion_penalty(x, concept)
        proj = self.compatibility_projection.get(concept)
        if proj is None:
            return penalty
        return float(penalty + self.projection_penalty_weight * c7.sq_dist(x, proj))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = DualGeometryCompatibilityGrounder(
        ...
        projection_pull=float(config["projection_pull"]),
        manifold_projection_weight=float(config["manifold_projection_weight"]),
        readout_projection_weight=float(config["readout_projection_weight"]),
        family_projection_weight=float(config["family_projection_weight"]),
        identity_projection_weight=float(config["identity_projection_weight"]),
        projection_penalty_weight=float(config["projection_penalty_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C51 dual-geometry compatibility projection search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c51_dual_geometry_compatibility_projection_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for projection_pull in [0.08, 0.16]:
        for manifold_projection_weight in [0.8, 1.0]:
            for readout_projection_weight in [0.2, 0.4]:
                for family_projection_weight in [0.2]:
                    for identity_projection_weight in [0.2, 0.4]:
                        for projection_penalty_weight in [0.02]:
                            config = {
                                **base,
                                "projection_pull": projection_pull,
                                "manifold_projection_weight": manifold_projection_weight,
                                "readout_projection_weight": readout_projection_weight,
                                "family_projection_weight": family_projection_weight,
                                "identity_projection_weight": identity_projection_weight,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c51_dual_geometry_not_enough"
        core_answer = "A dual-geometry compatibility projection improves structure separation, but still does not produce a better retention-compatible family."
        main_open_gap = "dual_geometry_compatibility_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c51_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c51_retention_compatible_dual_geometry_found"
        )
        core_answer = (
            "A dual-geometry compatibility projection creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A dual-geometry compatibility projection reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c51"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC51_dual_geometry_compatibility_projection_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c51_beats_c42_retention_compatible": bool(
            ...
            "H2_c51_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
