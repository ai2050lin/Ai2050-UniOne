...
def main() -> None:
    ...
    memory_batch = make_synthetic_batch(config, batch_size=24, seed=31)
    interference_batch = make_synthetic_batch(config, batch_size=24, seed=73)
    interference_batch["novelty"] = torch.full_like(interference_batch["novelty"], 0.42)
    ...
    replay_metrics = {}
    for _ in range(260):
        replay_metrics = model.replay_from_trace(trace, lr=3.2e-3, replay_strength=1.25)
    ...
    if replay_recovery_ratio < 0.75:
        for _ in range(140):
            replay_metrics = model.replay_from_trace(trace, lr=2.8e-3, replay_strength=1.30)
        replay_succ_err, replay_proto_err = mean_state_error(model, trace)
        replay_recovery_ratio = (
            (drift_succ_err + drift_proto_err - replay_succ_err - replay_proto_err)
            / max(1e-8, (drift_succ_err + drift_proto_err))
    ...
    payload = {
        "meta": {
            ...
            "task_block": "Stage_ICSPB_Backbone_V2_Replay_Recovery_Breakthrough_Block",
        },
        "results": {
        ...
        "verdict": {
            "overall_pass": replay_recovery_ratio > 0.68,
            "strict_replay_pass": strict_replay_pass,
            "core_answer": (
                "This block targets the remaining replay gap directly by combining gate restoration, "
                "local canonical anchor restoration, and local operator restoration."
            ),
        },
    }

    out_file = TEMP / "icspb_backbone_v2_replay_recovery_breakthrough_block.json"
