...
"""
Task block D:
Scan a unified base + concept_offset + novelty_offset consolidation law.

The core idea is to treat continuous grounding as the dynamic version of

    concept = shared_base + individual_offset

and extend it to phase-2 learning:

    state = family_base + stable_offset + novelty_offset

The new law tries to protect the phase-1 stable offset subspace while routing
phase-2 novelty into a partially orthogonalized offset estimate.
"""
...
def project(centered: np.ndarray, basis: np.ndarray) -> np.ndarray:
    if basis.shape[1] == 0:
...
class BaseOffsetConsolidationGrounder:
    def __init__(
        self,
        ...
        self.base_refresh = float(base_refresh)
        ...
        self.phase2_score: Dict[str, float] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        elif family in self.family_basis:
            # Small family-base refresh from phase-2 data, but keep it weak to
            # protect old concept identities.
            self.family_basis[family] = self._ema(self.family_basis.get(family), x, family_count, self.base_refresh)
            self.family_count[family] = family_count + 1
    ...
    def _refresh_candidate(self, concept: str, family: str) -> None:
        ...
        # Bayesian-style shrinkage toward the family offset center.
        ...
        # Build a novelty trust score from the orthogonal component and the
        # distance to the closest stable concept in the same family.
        sibling_scores = []
        ...
        self.phase2_score[concept] = novelty
    ...
    def predict(self, x: np.ndarray) -> Tuple[str, str]:
        ...
        for concept, offset in self.phase2_candidate.items():
            ...
            score = 0.74 * proto.sq_dist(x, proto_x) + 0.26 * self.phase2_score.get(concept, 0.0)
        ...
        if novel_best is not None:
            ...
            if stable_score > threshold and novel_score + self.route_margin < stable_score:
                return family, novel_best
            if novel_score + self.route_margin < 0.92 * stable_score:
                return family, novel_best
...
def run_candidate(
    ...
    model = BaseOffsetConsolidationGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Base+offset consolidation scan for continuous grounding")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/continuous_input_grounding_base_offset_consolidation_scan_20260309.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    candidates = []
    for threshold_quantile in [0.75, 0.80, 0.85]:
        for threshold_bonus in [0.00, 0.05, 0.10]:
            for posterior_kappa in [0.5, 1.5, 3.0]:
                for parallel_scale in [0.60, 0.80, 1.00]:
                    for orth_scale in [0.80, 1.00, 1.20, 1.40]:
                        for route_margin in [-0.02, 0.00, 0.02]:
                            for base_refresh in [0.00, 0.02, 0.05]:
                                rows = []
                                ...
                                candidates.append(
                                    {
                                        ...
                                        "base_refresh": float(base_refresh),
                                        **summary,
