...
import test_stage_c56_phase_transition_meta_switching_search as c56
import test_stage_c57_feasible_manifold_geometry_search as c57
...
class HigherOrderFeasibleManifoldGrounder(c57.FeasibleManifoldGeometryGrounder):
    def __init__(
        self,
        *args,
        meta_pull: float,
        meta_object_weight: float,
        meta_readout_weight: float,
        meta_transition_weight: float,
        meta_memory_weight: float,
        meta_identity_weight: float,
        meta_penalty_weight: float,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.meta_pull = meta_pull
        self.meta_object_weight = meta_object_weight
        self.meta_readout_weight = meta_readout_weight
        self.meta_transition_weight = meta_transition_weight
        self.meta_memory_weight = meta_memory_weight
        self.meta_identity_weight = meta_identity_weight
        self.meta_penalty_weight = meta_penalty_weight
        self.meta_center: Dict[str, np.ndarray] = {}

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)

        rows: List[np.ndarray] = []
        if concept in self.feasible_center:
            rows.append(self.feasible_center[concept].astype(np.float32))
        if concept in self.phase_object_state:
            rows.append(self.meta_object_weight * self.phase_object_state[concept].astype(np.float32))
        if concept in self.phase_discriminative_state:
            rows.append(self.meta_readout_weight * self.phase_discriminative_state[concept].astype(np.float32))
        if concept in self.phase_transition_state:
            rows.append(self.meta_transition_weight * self.phase_transition_state[concept].astype(np.float32))
        if concept in self.retention_state:
            rows.append(self.meta_memory_weight * self.retention_state[concept].astype(np.float32))
        if concept in self.identity_preserve_map:
            rows.append(self.meta_identity_weight * self.identity_preserve_map[concept].astype(np.float32))

        direction = self.update_direction_memory.get(concept)
        if direction is not None and rows:
            anchor = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
            rows.append(anchor + 0.08 * normalize(direction))

        if not rows:
            return

        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.meta_center.get(concept)
        self.meta_center[concept] = (
            target
            if prev is None
            else ((1.0 - self.meta_pull) * prev + self.meta_pull * target).astype(np.float32)
        )

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        penalty = super()._fusion_penalty(x, concept)
        meta = self.meta_center.get(concept)
        feasible = self.feasible_center.get(concept)
        if meta is None or feasible is None:
            return penalty
        meta_cost = c7.sq_dist(x, meta)
        align_cost = c7.sq_dist(meta, feasible)
        return float(penalty + self.meta_penalty_weight * (0.6 * meta_cost + 0.4 * align_cost))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    allowed.update(inspect.signature(c56.PhaseTransitionMetaSwitchingGrounder.__init__).parameters)
    allowed.update(inspect.signature(c57.FeasibleManifoldGeometryGrounder.__init__).parameters)
    allowed.update(inspect.signature(HigherOrderFeasibleManifoldGrounder.__init__).parameters)
    ...
    model = HigherOrderFeasibleManifoldGrounder(**model_config)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C58 higher-order feasible manifold search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c58_higher_order_feasible_manifold_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    stage_c57 = json.loads(
        (TEMP_DIR / "stage_c57_feasible_manifold_geometry_search_20260312.json").read_text(encoding="utf-8")
    )

    base = dict(stage_c57["best_objective_candidate"]["config"])
    ...
    for meta_pull in [0.08, 0.18]:
        for meta_object_weight in [0.25, 0.45]:
            for meta_readout_weight in [0.25, 0.45]:
                for meta_transition_weight in [0.20, 0.40]:
                    for meta_memory_weight in [0.20, 0.40]:
                        for meta_identity_weight in [0.20, 0.40]:
                            for meta_penalty_weight in [0.02, 0.05]:
                                config = {
                                    **base,
                                    "meta_pull": meta_pull,
                                    "meta_object_weight": meta_object_weight,
                                    "meta_readout_weight": meta_readout_weight,
                                    "meta_transition_weight": meta_transition_weight,
                                    "meta_memory_weight": meta_memory_weight,
                                    "meta_identity_weight": meta_identity_weight,
                                    "meta_penalty_weight": meta_penalty_weight,
                                }
                                rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c58_higher_order_feasible_not_enough"
        core_answer = "A higher-order feasible manifold sharpens meta-admissibility, but still does not produce a better retention-compatible family."
        main_open_gap = "higher_order_feasible_manifold_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c58_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c58_retention_compatible_higher_order_feasible_found"
        )
        core_answer = (
            "A higher-order feasible manifold creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A higher-order feasible manifold reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c58"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC58_higher_order_feasible_manifold_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c58_beats_c42_retention_compatible": bool(
            ...
            "H2_c58_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
