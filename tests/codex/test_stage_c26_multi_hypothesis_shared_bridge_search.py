...
import test_stage_c24_two_stage_identity_concept_search as c24
import test_stage_c25_factorized_representation_search as c25


ROOT = Path(__file__).resolve().parents[2]
TEMP_DIR = ROOT / "tests" / "codex_temp"
IDENTITY_IDX = np.array([0, 1, 4, 5, 8, 9, 12, 13, 16, 17, 20, 21], dtype=np.int64)
CONCEPT_IDX = np.array([2, 3, 6, 7, 10, 11, 14, 15, 18, 19, 22, 23], dtype=np.int64)


def restore_full(identity_part: np.ndarray, concept_part: np.ndarray) -> np.ndarray:
    full = np.zeros(24, dtype=np.float32)
    full[IDENTITY_IDX] = identity_part.astype(np.float32)
    full[CONCEPT_IDX] = concept_part.astype(np.float32)
    return full


def low_rank_project(x: np.ndarray) -> np.ndarray:
    rows = []
    for offset in range(0, 24, 4):
        rows.append(np.mean(x[offset : offset + 4], dtype=np.float32))
    return np.array(rows, dtype=np.float32)


class MultiHypothesisSharedBridgeGrounder(c24.TwoStageIdentityConceptGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        stage1_identity_weight: float,
        stage2_concept_weight: float,
        stage1_topk: int,
        mode: str,
        bridge_pull: float,
        bridge_weight: float,
        ...
        super().__init__(
            ...
            stage1_identity_weight=stage1_identity_weight,
            stage2_concept_weight=stage2_concept_weight,
            stage1_topk=stage1_topk,
        )
        self.mode = mode
        self.bridge_pull = bridge_pull
        self.bridge_weight = bridge_weight
        self.coupling_weight = coupling_weight
        self.identity_factor_state: Dict[str, np.ndarray] = {}
        self.concept_factor_state: Dict[str, np.ndarray] = {}
        self.bridge_state: Dict[str, np.ndarray] = {}

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        x_id, x_con = c25.split_subspaces(x)
        prev_id = self.identity_factor_state.get(concept)
        self.identity_factor_state[concept] = x_id if prev_id is None else (0.86 * prev_id + 0.14 * x_id).astype(np.float32)
        prev_con = self.concept_factor_state.get(concept)
        self.concept_factor_state[concept] = x_con if prev_con is None else (0.91 * prev_con + 0.09 * x_con).astype(np.float32)

        id_state = self.identity_factor_state.get(concept)
        con_state = self.concept_factor_state.get(concept)
        if id_state is None or con_state is None:
            return
        bridge_target = restore_full(id_state, con_state)
        pair = self.pair_state.get(concept)
        retention = self.retention_state.get(concept)
        if pair is not None:
            bridge_target = (0.7 * bridge_target + 0.3 * pair).astype(np.float32)
        if retention is not None:
            bridge_target = (0.8 * bridge_target + 0.2 * retention).astype(np.float32)
        prev_bridge = self.bridge_state.get(concept)
        self.bridge_state[concept] = (
            bridge_target
            if prev_bridge is None
            else ((1.0 - self.bridge_pull) * prev_bridge + self.bridge_pull * bridge_target).astype(np.float32)
        )

    def _bridge_penalty(self, x: np.ndarray, concept: str) -> float:
        id_state = self.identity_factor_state.get(concept)
        con_state = self.concept_factor_state.get(concept)
        bridge = self.bridge_state.get(concept)
        if id_state is None or con_state is None or bridge is None:
            return 0.0
        x_id, x_con = c25.split_subspaces(x)
        if self.mode == "soft_bridge_additive":
            return float(
                self.bridge_weight * c7.sq_dist(x, bridge)
                + self.coupling_weight * (c7.sq_dist(x_id, id_state) + c7.sq_dist(x_con, con_state))
            )
        if self.mode == "low_rank_bridge":
            return float(
                self.bridge_weight * c7.sq_dist(low_rank_project(x), low_rank_project(bridge))
                + self.coupling_weight * 0.5 * (c7.sq_dist(x_id, id_state) + c7.sq_dist(x_con, con_state))
            )
        if self.mode == "orthogonal_residual":
            recon = restore_full(id_state, con_state)
            residual = x.astype(np.float32) - recon
            return float(
                self.bridge_weight * c7.sq_dist(residual, bridge - recon)
                + self.coupling_weight * abs(float(np.dot(id_state, con_state[: id_state.shape[0]]) / (1e-6 + np.linalg.norm(id_state) * np.linalg.norm(con_state[: id_state.shape[0]]))))
            )
        if self.mode == "adaptive_balance":
            id_e = c7.sq_dist(x_id, id_state)
            con_e = c7.sq_dist(x_con, con_state)
            alpha = id_e / max(1e-6, id_e + con_e)
            return float(self.bridge_weight * c7.sq_dist(x, bridge) + self.coupling_weight * (alpha * id_e + (1.0 - alpha) * con_e))
        return 0.0
    ...
    def _concept_score(
        ...
        score += self._bridge_penalty(x, concept)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = MultiHypothesisSharedBridgeGrounder(
        ...
        stage1_identity_weight=float(config["stage1_identity_weight"]),
        stage2_concept_weight=float(config["stage2_concept_weight"]),
        stage1_topk=int(config["stage1_topk"]),
        mode=str(config["mode"]),
        bridge_pull=float(config["bridge_pull"]),
        bridge_weight=float(config["bridge_weight"]),
        coupling_weight=float(config["coupling_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C26 multi-hypothesis shared bridge search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c26_multi_hypothesis_shared_bridge_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c24 = json.loads((TEMP_DIR / "stage_c24_two_stage_identity_concept_search_20260311.json").read_text(encoding="utf-8"))
    stage_c15 = json.loads((TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8"))
    base = dict(stage_c24["best_objective_candidate"]["config"])
    ...
    for mode in ["soft_bridge_additive", "low_rank_bridge", "orthogonal_residual", "adaptive_balance"]:
        for bridge_pull in [0.08, 0.18]:
            for bridge_weight in [0.03, 0.08]:
                for coupling_weight in [0.02, 0.06]:
                    config = {
                        **base,
                        "mode": mode,
                        "bridge_pull": bridge_pull,
                    ...
                    prev_family = family_best.get(mode)
                    if prev_family is None or candidate["objective"] > prev_family["objective"]:
                        family_best[mode] = candidate
    ...
    if best_retention_compatible is None:
        status = "stage_c26_multi_hypothesis_not_enough"
        core_answer = "Multiple shared-bridge hypotheses were tested together, but none created a better retention-compatible family."
        main_open_gap = "shared_bridge_math_still_not_enough"
        ...
        status = (
            "stage_c26_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c26_retention_compatible_shared_bridge_found"
        )
        core_answer = (
            "A shared-bridge hypothesis creates a new retention-compatible family, but still does not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "A shared-bridge hypothesis reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c26"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC26_multi_hypothesis_shared_bridge_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_any_shared_bridge_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_any_shared_bridge_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
