...
import test_stage_c33_hybrid_controller_regime_switch_search as c33
...
class WriteVetoProtectedMemoryGrounder(c33.HybridControllerGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.memory_shield_weight = memory_shield_weight
        self.shield_pull = shield_pull
        self.protected_memory: Dict[str, np.ndarray] = {}

    def _write_risk(self, concept: str) -> float:
        pair = self.pair_state.get(concept)
        mem = self.memory_path.get(concept)
        nov = self.novelty_path.get(concept)
        if pair is None or mem is None:
            return 0.0
        risk = c7.sq_dist(pair, mem)
        if nov is not None:
            risk += 0.5 * c7.sq_dist(nov, mem)
        return float(risk)
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        veto = sigmoid(self.veto_strength * (risk - self.veto_threshold))

        prev_protected = self.protected_memory.get(concept)
        target_protected = mem.copy()
        self.protected_memory[concept] = (
            target_protected
            if prev_protected is None
            else ((1.0 - self.shield_pull) * prev_protected + self.shield_pull * target_protected).astype(np.float32)
        )

        # High write risk: pull identity/novelty states back toward protected memory.
        protected = self.protected_memory[concept]
        if pair is not None:
            self.identity_path[concept] = (
                (1.0 - 0.35 * veto) * self.identity_path[concept] + (0.35 * veto) * protected
            ).astype(np.float32)
        if nov is not None:
            self.novelty_path[concept] = (
                (1.0 - 0.45 * veto) * self.novelty_path[concept] + (0.45 * veto) * protected
            ).astype(np.float32)

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        base = super()._fusion_penalty(x, concept)
        protected = self.protected_memory.get(concept)
        mem = self.memory_path.get(concept)
        if protected is None or mem is None:
            return base
        risk = self._write_risk(concept)
        veto = sigmoid(self.veto_strength * (risk - self.veto_threshold))
        shield = self.memory_shield_weight * veto * (
            c7.sq_dist(x, protected) + 0.5 * c7.sq_dist(protected, mem)
        )
        return float(base + shield)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = WriteVetoProtectedMemoryGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C34 hybrid controller + write veto search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c34_hybrid_controller_write_veto_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c33 = json.loads((TEMP_DIR / "stage_c33_hybrid_controller_regime_switch_search_20260311.json").read_text(encoding="utf-8"))
    stage_c15 = json.loads((TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8"))
    base = dict(stage_c33["best_objective_candidate"]["config"])
    ...
    for veto_threshold in [10.0, 20.0]:
        for veto_strength in [0.15, 0.30]:
            for memory_shield_weight in [0.02, 0.06]:
                for shield_pull in [0.04, 0.10]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        status = "stage_c34_write_veto_not_enough"
        core_answer = "Write veto changes the hybrid controller tradeoff, but still does not create a better retention-compatible family."
        main_open_gap = "write_veto_still_not_enough"
        ...
        status = (
            "stage_c34_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c34_retention_compatible_write_veto_found"
        )
        core_answer = (
            "Write veto creates a new retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "Write veto reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c34"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC34_hybrid_controller_write_veto_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c34_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c34_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
