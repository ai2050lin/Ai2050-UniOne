...
OUTPUT_DIR = PROJECT_ROOT / "tests" / "codex_temp" / "stage125_adjective_neuron_basic_probe_20260323"
...
def run_analysis(
    *,
    ...
    return run_wordclass_analysis(
        target_lexical_type="adjective",
        experiment_id="stage125_adjective_neuron_basic_probe",
        title="Adjective 神经元基础探针",
        status_short="gpt2_adjective_neuron_probe_ready",
        input_dir=input_dir,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Adjective 神经元基础探针")
    parser.add_argument("--input-dir", default=str(STAGE119_OUTPUT_DIR), help="Stage119 输出目录")
    parser.add_argument("--output-dir", default=str(OUTPUT_DIR), help="Stage125 输出目录")
