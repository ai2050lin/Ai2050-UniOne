...
class QueryConditionedSelectiveReadoutGrounder(c44.ManifoldConditionedConsistencyHeadGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.query_readout_weight = query_readout_weight
    ...
    def _query_gate(self, x: np.ndarray, family: str, concept: str) -> Tuple[float, np.ndarray | None]:
        ...
        delta = (manifold_energy - self.query_readout_weight * readout_energy) / max(1e-4, self.query_temperature)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = QueryConditionedSelectiveReadoutGrounder(
        ...
        query_readout_weight=float(config["query_readout_weight"]),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C46 query-conditioned selective readout search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c46_query_conditioned_selective_readout_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for query_temperature in [0.18, 0.30]:
        for query_readout_weight in [0.95, 1.10]:
                    ...
                    for selective_penalty_weight in [0.02, 0.05]:
                        config = {
                            **base,
                            ...
                            "query_readout_weight": query_readout_weight,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c46_query_conditioned_selective_readout_not_enough"
        core_answer = "A query-conditioned selective readout improves access control, but still does not produce a better retention-compatible family."
        main_open_gap = "query_conditioned_readout_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c46_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c46_retention_compatible_selective_readout_found"
        )
        core_answer = (
            "A query-conditioned selective readout creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A query-conditioned selective readout reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c46"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC46_query_conditioned_selective_readout_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c46_beats_c42_retention_compatible": bool(
            ...
            "H2_c46_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
