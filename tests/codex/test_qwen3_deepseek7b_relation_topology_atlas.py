#!/usr/bin/env python
"""
Build a relation-support-family attention-topology atlas for Qwen3-4B and DeepSeek-7B.

This extends the direct topology atlas from the three concept families to the
relation endpoint support families:
- descriptor
- male / female
- instance / category
- part / whole
...
def analyze_model(
    ...
    family_rows = relation_scale.support_families()

    concept_topology: Dict[str, np.ndarray] = {}
    concept_entropy: Dict[str, float] = {}
    for words in family_rows.values():
        for word in words:
            rows = []
            ents = []
    ...
    family_basis = {}
    family_summary = {}
    for family, words in family_rows.items():
    ...
    wrong_residuals = []
    support_flags = []
    per_family_match: Dict[str, List[bool]] = {family: [] for family in family_rows}
    for family, words in family_rows.items():
        for word in words:
            x = concept_topology[word]
            fit = {}
            for cand_family in family_rows.keys():
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build Qwen3/DeepSeek7B relation topology atlas")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/qwen3_deepseek7b_relation_topology_atlas_20260309.json",
    )
    args = ap.parse_args()
