...
import test_stage_c22_retention_stabilized_pairwise_identity_search as c22
...
class DecoupledIdentityConceptGrounder(c22.RetentionStabilizedPairwiseGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.identity_pull = identity_pull
        self.identity_mix_weight = identity_mix_weight
        self.concept_pull = concept_pull
        self.concept_mix_weight = concept_mix_weight
        self.identity_fast_state: Dict[str, np.ndarray] = {}
        self.concept_slow_state: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if pair is not None and margin is not None:
            fast_target = (0.55 * pair + 0.45 * margin).astype(np.float32)
            prev_fast = self.identity_fast_state.get(concept)
            if prev_fast is None:
                self.identity_fast_state[concept] = fast_target
            else:
                self.identity_fast_state[concept] = (
                    (1.0 - self.identity_pull) * prev_fast + self.identity_pull * fast_target
                ).astype(np.float32)
        if manifold is not None and retention is not None:
            slow_target = (0.6 * manifold + 0.4 * retention).astype(np.float32)
            prev_slow = self.concept_slow_state.get(concept)
            if prev_slow is None:
                self.concept_slow_state[concept] = slow_target
            else:
                self.concept_slow_state[concept] = (
                    (1.0 - self.concept_pull) * prev_slow + self.concept_pull * slow_target
                ).astype(np.float32)

    def _identity_energy(self, x: np.ndarray, concept: str) -> float:
        fast = self.identity_fast_state.get(concept)
        if fast is None:
            return 0.0
        return float(c7.sq_dist(x, fast))

    def _concept_memory_energy(self, x: np.ndarray, concept: str) -> float:
        slow = self.concept_slow_state.get(concept)
        if slow is None:
            return 0.0
        return float(c7.sq_dist(x, slow))
    ...
    def _concept_score(
        ...
        score += self.identity_mix_weight * self._identity_energy(x, concept)
        score += self.concept_mix_weight * self._concept_memory_energy(x, concept)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = DecoupledIdentityConceptGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C23 decoupled identity / concept memory search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c23_decoupled_identity_concept_memory_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c22 = json.loads((TEMP_DIR / "stage_c22_retention_stabilized_pairwise_identity_search_20260311.json").read_text(encoding="utf-8"))
    stage_c15 = json.loads((TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8"))
    base = dict(stage_c22["best_objective_candidate"]["config"])
    ...
    for identity_pull in [0.10, 0.25]:
        for identity_mix_weight in [0.01, 0.03]:
            for concept_pull in [0.04, 0.10]:
                for concept_mix_weight in [0.03, 0.08]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        status = "stage_c23_decoupling_not_enough"
        core_answer = "Decoupling identity head and concept memory softens the conflict, but still does not create a better retention-compatible family."
        main_open_gap = "identity_concept_decoupling_still_not_enough"
        ...
        status = (
            "stage_c23_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c23_retention_compatible_decoupled_family_found"
        )
        core_answer = (
            "Decoupling identity head and concept memory creates a new retention-compatible family, but still does not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "Decoupling identity head and concept memory reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c23"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC23_decoupled_identity_concept_memory_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c23_improves_over_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c23_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
