...
"""
Task block D:
state-dependent consolidation law scan.

This script merges the three strongest clues so far:
- residual novelty gate
- annealed Bayesian posterior
- controller-level scheduling

The controller now decides:
- gate openness
- posterior trust
- write-back strength
- stable-route protection
- raw-vs-Bayes blend

The goal is to see whether this unified law can restore novel learning while
keeping the current best overall barrier close to zero.
"""
...
class StateDependentConsolidationGrounder:
    def __init__(
        self,
        controller_seed: int,
        base_alpha: float,
        ...
        self.controller_u = rng.normal(scale=0.48, size=(7, 7)).astype(np.float32)
        self.controller_w = rng.normal(scale=0.48, size=(5, 7)).astype(np.float32)
        self.controller_bias = np.zeros(5, dtype=np.float32)
    ...
    def _controller_state(self, family: str, concept: str) -> np.ndarray:
        posterior = self.phase2_post[concept]
        raw_mean = self.phase2_mean[concept]
        ...
        raw_post_gap = float(proto.sq_dist(raw_mean, posterior))
        history = np.array(self.family_novelty[family] or [0.0], dtype=np.float32)
        novelty_quantile = float(np.quantile(history, 0.70))
        state = np.array(
            [
                novelty,
        ...
        mean = np.array([0.50, 0.30, 0.08, 0.50, 0.50, 0.05, 0.45], dtype=np.float32)
        scale = np.array([0.60, 0.25, 0.12, 0.30, 0.35, 0.08, 0.40], dtype=np.float32)
    ...
    def _candidate_artifacts(
        ...
        for concept in self.phase2_post:
            family = proto.concept_family(concept)
            state = self._controller_state(family, concept)
            gate_open, posterior_trust, write_strength, protect, blend = [
                float(v) for v in self._controller_outputs(state, bias=bias, gain=gain)
            ]

            history = np.array(self.family_novelty[family] or [0.0], dtype=np.float32)
            novelty = self.phase2_novelty[concept]
            residual_gate = novelty > float(np.quantile(history, 0.70))
            threshold_boost[family] = max(
                threshold_boost[family],
                0.12 * max(0.0, protect - 0.55) * (1.0 - 0.45 * state[4]),
            )

            if not residual_gate and gate_open <= self.gate_bias:
                continue

            raw_mean = self.phase2_mean[concept]
            posterior = self.phase2_post[concept]
            blended = ((1.0 - blend) * raw_mean + blend * posterior).astype(np.float32)
            trust_center = ((1.0 - posterior_trust) * raw_mean + posterior_trust * blended).astype(np.float32)
            alpha = self.base_alpha * (0.30 + 0.95 * write_strength) * (0.55 + 0.55 * gate_open)
            alpha = float(np.clip(alpha, 0.03, 0.24))
            prev = candidate_proto.get(concept)
            candidate_proto[concept] = ((1.0 - alpha) * prev + alpha * trust_center).astype(np.float32) if prev is not None else trust_center

        return candidate_proto, threshold_boost

    def _predict_with_artifacts(
        self,
        x: np.ndarray,
        candidate_proto: Dict[str, np.ndarray],
        ...
        best_score = float("inf")
        for concept, center in candidate_proto.items():
            if proto.concept_family(concept) != family:
                continue
    ...
    def _objective(
        ...
        candidate_proto, threshold_boost = self._candidate_artifacts(bias=bias, gain=gain)
        novel = 0
        novel_total = 0
        retention = 0
        retention_total = 0
        ...
        for _ in range(6):
            for family, concepts in proto.PHASE2.items():
                for concept in concepts:
                    x = proto.sample_continuous_input(rng, concept, noise, dropout_p)
                    _, pred_concept = self._predict_with_artifacts(x, candidate_proto, threshold_boost)
        ...
        return float(1.85 * novel_acc + 0.80 * retention_acc + 1.25 * overall_acc)
    ...
    def predict(self, x: np.ndarray) -> Tuple[str, str]:
        candidate_proto, threshold_boost = self._candidate_artifacts()
        return self._predict_with_artifacts(x, candidate_proto, threshold_boost)
...
def run_candidate(
    ...
    model = StateDependentConsolidationGrounder(
    ...
    for _ in range(3):
        for family, concepts in proto.PHASE2.items():
            for concept in concepts:
                ...
                direct.train(x, family, concept)
                model.train(x, family, concept)
    model.adapt_controller(seed=seed + 503, noise=noise, dropout_p=dropout_p)
    ...
    for _ in range(18):
        for family, concepts in proto.PHASE2.items():
            for concept in concepts:
                ...
                direct.train(x, family, concept)
                model.train(x, family, concept)
    model.adapt_controller(seed=seed + 1503, noise=noise, dropout_p=dropout_p)
    ...
    return {
        ...
        "bias_gate": float(model.controller_bias[0]),
        "bias_trust": float(model.controller_bias[1]),
        "bias_write": float(model.controller_bias[2]),
        "bias_protect": float(model.controller_bias[3]),
        "bias_blend": float(model.controller_bias[4]),
    }
...
def main() -> None:
    ap = argparse.ArgumentParser(description="State-dependent consolidation law scan for continuous grounding")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/continuous_input_grounding_state_dependent_consolidation_scan_20260309.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    candidates = []
    for controller_seed in [3, 9]:
        for base_alpha in [0.08, 0.12]:
            for gate_bias in [0.40, 0.50]:
                for adapt_lr in [0.12, 0.18]:
                    rows = []
