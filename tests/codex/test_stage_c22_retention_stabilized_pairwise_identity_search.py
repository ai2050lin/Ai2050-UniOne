...
import test_stage_c21_pairwise_same_object_identity_test as c21
...
class RetentionStabilizedPairwiseGrounder(c21.PairwiseSameObjectGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.retention_pull = retention_pull
        self.retention_weight = retention_weight
        self.retention_state: Dict[str, np.ndarray] = {}

    def _retention_target(self, concept: str) -> np.ndarray | None:
        parts: List[np.ndarray] = []
        for value in [
            self.persistence_state.get(concept),
            self.read_slot.get(concept),
            self.margin_state.get(concept),
            self.pair_state.get(concept),
        ]:
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        target = self._retention_target(concept)
        if target is None:
            return
        prev = self.retention_state.get(concept)
        if prev is None:
            self.retention_state[concept] = target
        else:
            self.retention_state[concept] = (
                (1.0 - self.retention_pull) * prev + self.retention_pull * target
            ).astype(np.float32)

    def _retention_energy(self, x: np.ndarray, concept: str) -> float:
        state = self.retention_state.get(concept)
        if state is None:
            return 0.0
        return float(c7.sq_dist(x, state))
    ...
    def _concept_score(
        ...
        score += self.retention_weight * self._retention_energy(x, concept)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = RetentionStabilizedPairwiseGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C22 retention-stabilized pairwise identity search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c22_retention_stabilized_pairwise_identity_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c21 = json.loads((TEMP_DIR / "stage_c21_pairwise_same_object_identity_test_20260311.json").read_text(encoding="utf-8"))
    stage_c15 = json.loads((TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8"))
    base = dict(stage_c21["best_objective_candidate"]["config"])
    ...
    for retention_pull in [0.06, 0.14]:
        for retention_weight in [0.02, 0.06]:
            for extra_retention_replay in [1, 2]:
                config = {
                    **base,
                    "retention_pull": retention_pull,
                    "retention_weight": retention_weight,
                    "extra_retention_replay": extra_retention_replay,
                }
                rows = []
    ...
    if best_retention_compatible is None:
        status = "stage_c22_retention_stabilization_not_enough"
        core_answer = "Retention-stabilized pairwise identity improves retention support, but still does not create a better retention-compatible family."
        main_open_gap = "pairwise_identity_retention_tradeoff_still_open"
        ...
        status = (
            "stage_c22_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c22_retention_compatible_pairwise_identity_found"
        )
        core_answer = (
            "Retention-stabilized pairwise identity creates a new retention-compatible family, but still does not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "Retention-stabilized pairwise identity reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c22"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC22_retention_stabilized_pairwise_identity_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c22_improves_over_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c22_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
