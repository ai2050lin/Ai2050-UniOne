...
class GlobalConstraintConsensusGrounder(c16.IdentityGraphConsensusGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.negative_margin_weight = negative_margin_weight
        self.family_center_weight = family_center_weight
        self.object_anchor: Dict[str, np.ndarray] = {}

    def _anchor_target(self, concept: str) -> np.ndarray | None:
        parts = []
        ...
        if full_proto is not None:
            parts.append((0.30, full_proto))
        if read_slot is not None:
            parts.append((0.25, read_slot))
        if persist is not None:
            parts.append((0.25, persist))
        if node is not None:
            parts.append((0.20, node))
        if not parts:
            return None
        ...
        for weight, value in parts:
            ...
            total += float(weight)
        return (acc / max(1e-6, total)).astype(np.float32)

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        target = self._anchor_target(concept)
        if target is None:
            return
        prev = self.object_anchor.get(concept)
        if prev is None:
            self.object_anchor[concept] = target
        else:
            self.object_anchor[concept] = (
                (1.0 - self.anchor_pull) * prev + self.anchor_pull * target
            ).astype(np.float32)

    def _nearest_negative_margin(self, family: str, concept: str, anchor: np.ndarray) -> float:
        negatives = []
        for other, other_family in self.concept_family.items():
            if other == concept or other_family != family:
                continue
            other_anchor = self.object_anchor.get(other)
            if other_anchor is None:
                continue
            negatives.append(c7.sq_dist(anchor, other_anchor))
        if not negatives:
            return 0.0
    ...
    def _concept_score(
        ...
        if anchor is not None:
            score += self.anchor_weight * c7.sq_dist(x, anchor)
            score += self.negative_margin_weight * self._nearest_negative_margin(family, concept, anchor)
        if anchor is not None and family_center is not None:
            score += self.family_center_weight * c7.sq_dist(anchor, family_center)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = GlobalConstraintConsensusGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C18 global constraint discriminative head search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c18_global_constraint_discriminative_head_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    for anchor_pull in [0.10, 0.25]:
        for anchor_weight in [0.03, 0.08]:
            for negative_margin_weight in [0.02, 0.06]:
                for family_center_weight in [0.0, 0.03]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        status = "stage_c18_global_constraint_not_retention_compatible"
        core_answer = "Global object constraint improves the discriminative head framing, but still cannot beat the current retention-compatible family."
        main_open_gap = "global_constraint_head_still_not_enough"
        ...
        status = (
            "stage_c18_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c18_global_constraint_family_strengthened"
        )
        core_answer = (
            "Global object constraint strengthens the retention-compatible family, but still does not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "Global object constraint reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c18"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC18_global_constraint_discriminative_head_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c18_improves_over_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c18_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
