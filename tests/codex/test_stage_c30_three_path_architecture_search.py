...
class ThreePathArchitectureGrounder(c23.DecoupledIdentityConceptGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.fusion_identity_weight = fusion_identity_weight
        self.fusion_memory_weight = fusion_memory_weight
        self.fusion_novelty_weight = fusion_novelty_weight
        self.controller_temperature = controller_temperature
        self.identity_path: Dict[str, np.ndarray] = {}
        self.memory_path: Dict[str, np.ndarray] = {}
        self.novelty_path: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if pair is not None:
            prev = self.identity_path.get(concept)
            self.identity_path[concept] = pair if prev is None else (0.84 * prev + 0.16 * pair).astype(np.float32)
        if retention is not None:
            prev = self.memory_path.get(concept)
            self.memory_path[concept] = retention if prev is None else (0.93 * prev + 0.07 * retention).astype(np.float32)
        if manifold is not None:
            prev = self.novelty_path.get(concept)
            self.novelty_path[concept] = manifold if prev is None else (0.78 * prev + 0.22 * manifold).astype(np.float32)

    def _path_energies(self, x: np.ndarray, concept: str) -> Tuple[float, float, float]:
        id_state = self.identity_path.get(concept)
        mem_state = self.memory_path.get(concept)
        nov_state = self.novelty_path.get(concept)
        id_e = 0.0 if id_state is None else c7.sq_dist(x, id_state)
        mem_e = 0.0 if mem_state is None else c7.sq_dist(x, mem_state)
        nov_e = 0.0 if nov_state is None else c7.sq_dist(x, nov_state)
        return float(id_e), float(mem_e), float(nov_e)

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        id_e, mem_e, nov_e = self._path_energies(x, concept)
        energies = np.array([id_e, mem_e, nov_e], dtype=np.float64)
        weights = np.exp(-(energies - np.min(energies)) / max(1e-6, self.controller_temperature))
        weights = weights / np.sum(weights)
        controller_mix = float(np.sum(weights * energies))
        explicit_mix = float(
            self.fusion_identity_weight * id_e
            + self.fusion_memory_weight * mem_e
            + self.fusion_novelty_weight * nov_e
        )
        return 0.5 * controller_mix + explicit_mix
    ...
    def _concept_score(
        ...
        score += self._fusion_penalty(x, concept)
        return float(score)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = ThreePathArchitectureGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C30 three-path architecture search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c30_three_path_architecture_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    for fusion_identity_weight in [0.15, 0.30]:
        for fusion_memory_weight in [0.15, 0.35]:
            for fusion_novelty_weight in [0.05, 0.15]:
                for controller_temperature in [0.12, 0.25]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        status = "stage_c30_three_path_not_enough"
        core_answer = "The three-path architecture changes the coupling pattern, but still does not create a better retention-compatible family."
        main_open_gap = "three_path_architecture_still_not_enough"
        ...
        status = (
            "stage_c30_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c30_retention_compatible_three_path_found"
        )
        core_answer = (
            "The three-path architecture creates a new retention-compatible family, but still does not reach the strong direct closure target."
            if best_compatible_consistency < strong_multimodal_target
            else "The three-path architecture reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c30"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC30_three_path_architecture_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c30_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c30_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
