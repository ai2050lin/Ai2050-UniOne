...
import test_stage_c38_identity_preserving_external_memory_law_search as c38
...
class TransitionAwareMetaLawGrounder(c38.IdentityPreservingExternalMemoryLawGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.transition_pull = transition_pull
        self.switch_penalty_weight = switch_penalty_weight
        self.transition_smooth_weight = transition_smooth_weight
        self.transition_identity_bonus = transition_identity_bonus
        self.previous_rule: Dict[str, str] = {}
        self.transition_state: Dict[str, np.ndarray] = {}

    def _rule_vector(self, rule: str, concept: str) -> np.ndarray | None:
        ...
        if rule == "identity" and identity is not None:
            return identity
        if rule == "memory" and ext is not None:
            return ext
        if rule == "novelty" and novelty is not None:
            return novelty
        if rule == "stabilize":
            rows = [state for state in (identity, ext, memory) if state is not None]
            if rows:
                return np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        return None

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        prev_rule = self.rule_state.get(concept)
        super().train(x, family, concept)
        rule = self.rule_state.get(concept)
        if rule is None:
            return
        self.previous_rule[concept] = prev_rule or rule

        prev_vec = self._rule_vector(prev_rule, concept) if prev_rule is not None else None
        cur_vec = self._rule_vector(rule, concept)
        if cur_vec is not None:
            target = cur_vec if prev_vec is None else (0.55 * cur_vec + 0.45 * prev_vec).astype(np.float32)
            prev_state = self.transition_state.get(concept)
            self.transition_state[concept] = (
                target
                if prev_state is None
                else ((1.0 - self.transition_pull) * prev_state + self.transition_pull * target).astype(np.float32)
            )

        transition = self.transition_state.get(concept)
        identity = self.identity_path.get(concept)
        novelty = self.novelty_path.get(concept)
        preserve = self.identity_preserve_map.get(concept)
        if transition is None or preserve is None:
            return

        switched = prev_rule is not None and prev_rule != rule
        if switched:
            if identity is not None:
                target = (1.0 - self.transition_identity_bonus) * transition + self.transition_identity_bonus * preserve
                self.identity_path[concept] = (
                    (1.0 - self.transition_smooth_weight) * identity + self.transition_smooth_weight * target
                ).astype(np.float32)
            if novelty is not None:
                self.novelty_path[concept] = (
                    (1.0 - 0.5 * self.transition_smooth_weight) * novelty
                    + (0.5 * self.transition_smooth_weight) * transition
                ).astype(np.float32)

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        base = super()._fusion_penalty(x, concept)
        rule = self.rule_state.get(concept)
        prev_rule = self.previous_rule.get(concept)
        transition = self.transition_state.get(concept)
        if rule is None or prev_rule is None or transition is None:
            return base
        switch_cost = self.switch_penalty_weight if rule != prev_rule else 0.0
        return float(base + switch_cost * c7.sq_dist(x, transition))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = TransitionAwareMetaLawGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C39 transition-aware meta-law search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c39_transition_aware_meta_law_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c38 = json.loads(
        (TEMP_DIR / "stage_c38_identity_preserving_external_memory_law_search_20260311.json").read_text(encoding="utf-8")
    )
    stage_c15 = json.loads(
        (TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8")
    )
    base = dict(stage_c38["best_objective_candidate"]["config"])
    ...
    for transition_pull in [0.12, 0.24]:
        for switch_penalty_weight in [0.03, 0.08]:
            for transition_smooth_weight in [0.10, 0.22]:
                for transition_identity_bonus in [0.20, 0.35]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c39_transition_meta_law_not_enough"
        core_answer = "Transition-aware meta-law improves switching control, but still does not produce a better retention-compatible family."
        main_open_gap = "transition_operator_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c39_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c39_retention_compatible_transition_meta_law_found"
        )
        core_answer = (
            "Transition-aware meta-law creates a retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "Transition-aware meta-law reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c39"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC39_transition_aware_meta_law_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c39_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c39_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
