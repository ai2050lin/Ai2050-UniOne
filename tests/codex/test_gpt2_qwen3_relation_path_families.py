#!/usr/bin/env python
"""
Compare relation path families across layers in GPT-2 and Qwen3.

Each relation type is represented by two aligned pairs:
- gender:   king->queen, man->woman
- hypernym: apple->fruit, cat->animal
- antonym:  hot->cold, big->small

We test whether relation paths align better in:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Test relation path families in GPT-2 and Qwen3")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/gpt2_qwen3_relation_path_families_20260308.json",
    )
    args = ap.parse_args()
