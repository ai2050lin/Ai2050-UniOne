...
class ManifoldReadoutCotrainingGrounder(c44.ManifoldConditionedConsistencyHeadGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        cotrain_pull: float,
        manifold_feedback_weight: float,
        readout_feedback_weight: float,
        coupling_penalty_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.cotrain_pull = cotrain_pull
        self.manifold_feedback_weight = manifold_feedback_weight
        self.readout_feedback_weight = readout_feedback_weight
        self.coupling_penalty_weight = coupling_penalty_weight
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if manifold is None or readout is None:
            return
        vote = self.temporal_vote_state.get(concept)
        preserve = self.identity_preserve_map.get(concept)
        family_state = self.family_manifold.get(family)

        bridge_rows = [
            (1.0 - self.readout_feedback_weight) * manifold + self.readout_feedback_weight * readout
        ]
        if vote is not None:
            bridge_rows.append(self.manifold_feedback_weight * vote)
        if preserve is not None:
            bridge_rows.append(0.5 * self.manifold_feedback_weight * preserve)
        if family_state is not None:
            bridge_rows.append(0.5 * self.readout_feedback_weight * family_state)
        bridge = np.mean(np.stack(bridge_rows, axis=0), axis=0).astype(np.float32)

        self.shared_manifold[concept] = (
            (1.0 - self.cotrain_pull) * manifold + self.cotrain_pull * bridge
        ).astype(np.float32)
        self.consistency_readout[concept] = (
            (1.0 - 0.5 * self.cotrain_pull) * readout + 0.5 * self.cotrain_pull * self.shared_manifold[concept]
        ).astype(np.float32)
        identity = self.identity_path.get(concept)
        if identity is not None:
            self.identity_path[concept] = (
                (1.0 - 0.5 * self.manifold_feedback_weight) * identity
                + 0.5 * self.manifold_feedback_weight * self.shared_manifold[concept]
            ).astype(np.float32)
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if manifold is None or readout is None:
            return penalty
        penalty += self.coupling_penalty_weight * c7.sq_dist(manifold, readout)
        mixed = (1.0 - self.readout_feedback_weight) * manifold + self.readout_feedback_weight * readout
        penalty += 0.5 * self.coupling_penalty_weight * c7.sq_dist(x, mixed.astype(np.float32))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = ManifoldReadoutCotrainingGrounder(
        ...
        cotrain_pull=float(config["cotrain_pull"]),
        manifold_feedback_weight=float(config["manifold_feedback_weight"]),
        readout_feedback_weight=float(config["readout_feedback_weight"]),
        coupling_penalty_weight=float(config["coupling_penalty_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C45 manifold-readout co-training search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c45_manifold_readout_cotraining_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    c44_best = stage_c44["best_objective_candidate"]["summary"]
    ...
    for cotrain_pull in [0.04, 0.08]:
        for manifold_feedback_weight in [0.05, 0.10]:
            for readout_feedback_weight in [0.10, 0.16]:
                for coupling_penalty_weight in [0.02, 0.05]:
                    config = {
                        **base,
                        "cotrain_pull": cotrain_pull,
                        "manifold_feedback_weight": manifold_feedback_weight,
                        "readout_feedback_weight": readout_feedback_weight,
                        "coupling_penalty_weight": coupling_penalty_weight,
                    }
                    rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c45_manifold_readout_cotraining_not_enough"
        core_answer = "A manifold-readout co-training law improves coordination, but still does not produce a better retention-compatible family."
        main_open_gap = "representation_readout_coupling_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c45_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c45_retention_compatible_cotraining_found"
        )
        core_answer = (
            "A manifold-readout co-training law creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A manifold-readout co-training law reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c45"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC45_manifold_readout_cotraining_search",
        },
        "headline_metrics": {
            ...
            "c44_best_objective_consistency": float(c44_best["crossmodal_consistency"]),
        ...
        "hypotheses": {
            "H1_c45_beats_c42_retention_compatible": bool(
            ...
            "H2_c45_beats_c44_best_objective": bool(
                best_objective["summary"]["crossmodal_consistency"] > float(c44_best["crossmodal_consistency"])
            ),
            "H3_c45_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
