...
import test_stage_c39_transition_aware_meta_law_search as c39
...
class LawConditionedLatentGrounder(c39.TransitionAwareMetaLawGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.latent_pull = latent_pull
        self.latent_identity_mix = latent_identity_mix
        self.latent_transition_mix = latent_transition_mix
        self.latent_penalty_weight = latent_penalty_weight
        self.shared_latent: Dict[str, np.ndarray] = {}

    def _update_shared_latent(self, concept: str) -> None:
        rows = []
        ...
        if identity is not None:
            rows.append(self.latent_identity_mix * identity.astype(np.float32))
        if transition is not None:
            rows.append(self.latent_transition_mix * transition.astype(np.float32))
        if ext is not None:
            rows.append(0.35 * ext.astype(np.float32))
        if memory is not None:
            rows.append(0.25 * memory.astype(np.float32))
        if not rows:
            return
        target = np.sum(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.shared_latent.get(concept)
        self.shared_latent[concept] = (
            target
            if prev is None
            else ((1.0 - self.latent_pull) * prev + self.latent_pull * target).astype(np.float32)
        )

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        self._update_shared_latent(concept)
        ...
        if rule in {"memory", "stabilize"} and identity is not None:
            self.identity_path[concept] = ((1.0 - 0.10) * identity + 0.10 * latent).astype(np.float32)
        if rule in {"identity", "stabilize"} and novelty is not None:
            self.novelty_path[concept] = ((1.0 - 0.08) * novelty + 0.08 * latent).astype(np.float32)
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if latent is None:
            return base
        return float(base + self.latent_penalty_weight * c7.sq_dist(x, latent))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = LawConditionedLatentGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C40 law-conditioned latent search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c40_law_conditioned_latent_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c39 = json.loads(
        (TEMP_DIR / "stage_c39_transition_aware_meta_law_search_20260311.json").read_text(encoding="utf-8")
    )
    stage_c15 = json.loads(
        (TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8")
    )
    base = dict(stage_c39["best_objective_candidate"]["config"])
    ...
    for latent_pull in [0.10, 0.22]:
        for latent_identity_mix in [0.35, 0.55]:
            for latent_transition_mix in [0.20, 0.40]:
                for latent_penalty_weight in [0.02, 0.06]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c40_law_conditioned_latent_not_enough"
        core_answer = "A cross-law shared latent changes the geometry class, but still does not produce a better retention-compatible family."
        main_open_gap = "shared_cross_law_coordinate_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c40_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c40_retention_compatible_law_conditioned_latent_found"
        )
        core_answer = (
            "A cross-law shared latent creates a retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A cross-law shared latent reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c40"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC40_law_conditioned_latent_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c40_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c40_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
