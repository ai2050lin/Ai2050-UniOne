...
class CompatibilityGeometryGrounder(c44.ManifoldConditionedConsistencyHeadGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        compatibility_pull: float,
        compatibility_readout_weight: float,
        compatibility_family_weight: float,
        compatibility_identity_weight: float,
        compatibility_penalty_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.compatibility_pull = compatibility_pull
        self.compatibility_readout_weight = compatibility_readout_weight
        self.compatibility_family_weight = compatibility_family_weight
        self.compatibility_identity_weight = compatibility_identity_weight
        self.compatibility_penalty_weight = compatibility_penalty_weight
        self.compatibility_state: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if manifold is not None:
            rows.append(manifold.astype(np.float32))
        readout = self.consistency_readout.get(concept)
        if readout is not None:
            rows.append(self.compatibility_readout_weight * readout.astype(np.float32))
        family_state = self.family_manifold.get(family)
        if family_state is not None:
            rows.append(self.compatibility_family_weight * family_state.astype(np.float32))
        identity = self.identity_preserve_map.get(concept)
        if identity is not None:
            rows.append(self.compatibility_identity_weight * identity.astype(np.float32))
        if not rows:
            return
        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.compatibility_state.get(concept)
        self.compatibility_state[concept] = (
            target
            if prev is None
            else ((1.0 - self.compatibility_pull) * prev + self.compatibility_pull * target).astype(np.float32)
        )

    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        penalty = super()._fusion_penalty(x, concept)
        compat = self.compatibility_state.get(concept)
        if compat is None:
            return penalty
        manifold = self.shared_manifold.get(concept)
        readout = self.consistency_readout.get(concept)
        penalty += self.compatibility_penalty_weight * c7.sq_dist(x, compat)
        if manifold is not None:
            penalty += 0.5 * self.compatibility_penalty_weight * c7.sq_dist(compat, manifold)
        if readout is not None:
            penalty += 0.5 * self.compatibility_penalty_weight * c7.sq_dist(compat, readout)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = CompatibilityGeometryGrounder(
        ...
        compatibility_pull=float(config["compatibility_pull"]),
        compatibility_readout_weight=float(config["compatibility_readout_weight"]),
        compatibility_family_weight=float(config["compatibility_family_weight"]),
        compatibility_identity_weight=float(config["compatibility_identity_weight"]),
        compatibility_penalty_weight=float(config["compatibility_penalty_weight"]),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C50 compatibility geometry search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c50_compatibility_geometry_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for compatibility_pull in [0.08, 0.16]:
        for compatibility_readout_weight in [0.8, 1.0]:
            for compatibility_family_weight in [0.2, 0.4]:
                for compatibility_identity_weight in [0.2, 0.4]:
                    for compatibility_penalty_weight in [0.02, 0.05]:
                        config = {
                            **base,
                            "compatibility_pull": compatibility_pull,
                            "compatibility_readout_weight": compatibility_readout_weight,
                            "compatibility_family_weight": compatibility_family_weight,
                            "compatibility_identity_weight": compatibility_identity_weight,
                            "compatibility_penalty_weight": compatibility_penalty_weight,
                        }
                        rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c50_compatibility_geometry_not_enough"
        core_answer = "A compatibility geometry mediator improves structure matching, but still does not produce a better retention-compatible family."
        main_open_gap = "compatibility_geometry_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c50_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c50_retention_compatible_compatibility_geometry_found"
        )
        core_answer = (
            "A compatibility geometry mediator creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A compatibility geometry mediator reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c50"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC50_compatibility_geometry_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c50_beats_c42_retention_compatible": bool(
            ...
            "H2_c50_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
