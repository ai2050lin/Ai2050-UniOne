...
class HybridControllerGrounder(c30.ThreePathArchitectureGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.mode_identity_bias = mode_identity_bias
        self.mode_memory_bias = mode_memory_bias
        self.mode_novelty_bias = mode_novelty_bias
        self.mode_margin = mode_margin

    def _mode_logits(self, x: np.ndarray, concept: str) -> np.ndarray:
        ...
        memory_conf = 0.0 if retention is None else 1.0 / max(1e-4, c7.sq_dist(x, retention))
        novelty_conf = 0.0 if novelty is None else 1.0 / max(1e-4, c7.sq_dist(x, novelty))
        return np.array(
            [
                self.mode_identity_bias * identity_conf - 0.2 * id_e,
                self.mode_memory_bias * memory_conf - 0.2 * mem_e,
                self.mode_novelty_bias * novelty_conf - 0.2 * nov_e,
                0.5 * (identity_conf + memory_conf) - self.mode_margin * abs(id_e - mem_e),
            ],
            dtype=np.float64,
        )
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        id_e, mem_e, nov_e = self._path_energies(x, concept)
        logits = self._mode_logits(x, concept)
        mode = int(np.argmax(logits))
        if mode == 0:
            return float(self.fusion_identity_weight * id_e + 0.25 * mem_e + 0.10 * nov_e)
        if mode == 1:
            return float(self.fusion_memory_weight * mem_e + 0.20 * id_e + 0.10 * nov_e)
        if mode == 2:
            return float(self.fusion_novelty_weight * nov_e + 0.20 * id_e + 0.20 * mem_e)
        return float(
            0.45 * self.fusion_identity_weight * id_e
            + 0.45 * self.fusion_memory_weight * mem_e
            + 0.10 * self.fusion_novelty_weight * nov_e
        )
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = HybridControllerGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C33 hybrid controller / regime-switch search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c33_hybrid_controller_regime_switch_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    for mode_identity_bias in [0.8, 1.4]:
        for mode_memory_bias in [0.8, 1.4]:
            for mode_novelty_bias in [0.2, 0.5]:
                for mode_margin in [0.05, 0.15]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        status = "stage_c33_hybrid_not_enough"
        core_answer = "The hybrid regime-switching controller changes the failure shape, but still does not create a better retention-compatible family."
        main_open_gap = "hybrid_controller_still_not_enough"
        ...
        status = (
            "stage_c33_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c33_retention_compatible_hybrid_found"
        )
        core_answer = (
            "The hybrid regime-switching controller creates a new retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "The hybrid regime-switching controller reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c33"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC33_hybrid_controller_regime_switch_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c33_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c33_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
