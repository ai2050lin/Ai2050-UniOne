...
def main() -> None:
    ...
    memory_batch = make_synthetic_batch(config, batch_size=24, seed=11)
    interference_batch = make_synthetic_batch(config, batch_size=24, seed=37)
    interference_batch["novelty"] = torch.full_like(interference_batch["novelty"], 0.35)
    interference_batch["retention"] = torch.full_like(interference_batch["retention"], 0.02)
    ...
    replay_metrics = {}
    for _ in range(160):
        replay_metrics = model.replay_from_trace(trace, lr=3e-3, replay_strength=1.0)
    ...
    replay_ready = replay_recovery_ratio > 0.60
    strict_replay_pass = (
        replay_recovery_ratio > 0.75
        and replay_metrics["stable_read"] >= 1.0
        and replay_metrics["theorem_survival"] >= 1.0
    )
    ...
    payload = {
        "meta": {
            ...
            "task_block": "Stage_ICSPB_Backbone_V2_Memory_Replay_Block",
        },
        "results": {
        ...
        "verdict": {
            "memory_replay_ready": replay_ready,
            "strict_replay_pass": strict_replay_pass,
            "core_answer": (
                "The current ICSPB-Backbone-v2-LargeOnline can implement constrained memory replay: "
                "it captures a replayable trace, suffers interference, and then recovers successor/protocol "
                "structure through guarded replay updates."
            ),
        },
    }

    out_file = TEMP / "icspb_backbone_v2_memory_replay_block.json"
