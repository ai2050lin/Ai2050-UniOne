#!/usr/bin/env python
"""
Build a full relation coupling atlas in GPT-2 and Qwen3.

Extends the previous coupling trace from 3 relation families to 6:
- gender
- hypernym
- antonym
- synonym
...
For each relation family we estimate how concept-basis stability couples to
relation alignment in representation space H and topology space T.
"""
...
def coupling_protocol(summary: Dict[str, float]) -> str:
    ht = float(summary["max_bridge_ht"])
    hh = float(summary["max_bridge_hh"])
    tt = float(summary["max_bridge_tt"])
    if tt >= ht and tt >= hh:
        return "tt"
    if ht >= hh:
        return "ht"
...
def analyze_model(model_name: str, model_path: str, dtype_name: str, prefer_cuda: bool) -> Dict[str, object]:
    ...
    for rel_name, spec in relation_specs().items():
        ...
        summary["coupling_protocol"] = coupling_protocol(summary)
        relations[rel_name] = {
            "pairs": spec["pairs"],
        ...
        atlas_rows.append(
            {
                "relation": rel_name,
                "protocol": summary["coupling_protocol"],
                "max_bridge_ht": summary["max_bridge_ht"],
                "max_bridge_hh": summary["max_bridge_hh"],
                "max_bridge_tt": summary["max_bridge_tt"],
                "topo_dominant_ratio": summary["topo_dominant_ratio"],
                "max_relation_align_topo": summary["max_relation_align_topo"],
                "max_relation_align_repr": summary["max_relation_align_repr"],
                "peak_endpoint_repr_basis": summary["peak_endpoint_repr_basis"],
                "peak_endpoint_topo_basis": summary["peak_endpoint_topo_basis"],
                "best_bridge_ht_layer": summary["best_bridge_ht_layers"][0],
                "best_bridge_tt_layer": summary["best_bridge_tt_layers"][0],
            }
        )
    ...
    global_summary = {
        "relation_count": len(atlas_rows),
        "protocol_counts": protocol_counts,
        "mean_topo_dominant_ratio": float(np.mean([row["topo_dominant_ratio"] for row in atlas_rows])),
        "mean_max_bridge_ht": float(np.mean([row["max_bridge_ht"] for row in atlas_rows])),
        "mean_max_bridge_hh": float(np.mean([row["max_bridge_hh"] for row in atlas_rows])),
        "mean_max_bridge_tt": float(np.mean([row["max_bridge_tt"] for row in atlas_rows])),
    }
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build a full relation coupling atlas in GPT-2 and Qwen3")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/gpt2_qwen3_relation_coupling_atlas_20260308.json",
    )
    args = ap.parse_args()
