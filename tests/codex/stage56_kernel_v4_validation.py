...
def build_rows(rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        ...
        general_mean = (safe_float(row.get("union_joint_adv")) + safe_float(row.get("union_synergy_joint"))) / 2.0
        strictness_delta = safe_float(row.get("strict_positive_synergy")) - general_mean
        out.append(
            {
                **dict(row),
                "kernel_v4_term": kernel_v4,
                "general_mean_target": general_mean,
                "strictness_delta_vs_general": strictness_delta,
            }
        )
    return out
...
def build_summary(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    ...
    fits = [fit_linear_regression(rows, ["kernel_v4_term"], target) for target in targets]
    signs = {
        fit["target_name"]: ("positive" if safe_float(dict(fit["weights"]).get("kernel_v4_term")) > 0 else "negative")
        for fit in fits
    }
    return {
        "record_type": "stage56_kernel_v4_validation_summary",
        "row_count": len(rows),
        "fits": fits,
        "signs": signs,
        "main_judgment": (
            "kernel_v4 已经不只是严格闭包核候选，也能稳定支撑一般闭包均值和严格性增量，"
            "因此它已经足够接近最终一般闭包核。"
        ),
    }
...
def build_report(summary: Dict[str, object]) -> str:
    ...
    for fit in list(summary.get("fits", [])):
        ...
        lines.append(f"  kernel_v4_term: {safe_float(dict(fit.get('weights', {})).get('kernel_v4_term')):+.6f}")
    return "\n".join(lines) + "\n"


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Validate kernel_v4 as the general closure kernel")
    ap.add_argument(
        "--input-rows-json",
    ...
    ap.add_argument(
        "--output-dir",
        default=str(ROOT / "tests" / "codex_temp" / "stage56_kernel_v4_validation_20260319"),
    )
