...
class FeasibleManifoldGeometryGrounder(c56.PhaseTransitionMetaSwitchingGrounder):
    def __init__(
        self,
        *args,
        feasible_pull: float,
        feasible_object_weight: float,
        feasible_discriminative_weight: float,
        feasible_transition_weight: float,
        feasible_memory_weight: float,
        feasible_penalty_weight: float,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.feasible_pull = feasible_pull
        self.feasible_object_weight = feasible_object_weight
        self.feasible_discriminative_weight = feasible_discriminative_weight
        self.feasible_transition_weight = feasible_transition_weight
        self.feasible_memory_weight = feasible_memory_weight
        self.feasible_penalty_weight = feasible_penalty_weight
        self.feasible_center: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        transition_state = self.phase_transition_state.get(concept)
        protected = self._protected_center(concept)
        retention_state = self.retention_state.get(concept)
        transport_state = self.transport_state.get(concept)

        if object_state is not None:
            rows.append(self.feasible_object_weight * object_state.astype(np.float32))
        if discrim_state is not None:
            rows.append(self.feasible_discriminative_weight * discrim_state.astype(np.float32))
        if transition_state is not None:
            rows.append(self.feasible_transition_weight * transition_state.astype(np.float32))
        if protected is not None:
            rows.append(self.feasible_memory_weight * protected.astype(np.float32))
        if retention_state is not None:
            rows.append(0.35 * retention_state.astype(np.float32))
        if transport_state is not None:
            rows.append(0.25 * transport_state.astype(np.float32))

        if not rows:
            return

        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.feasible_center.get(concept)
        self.feasible_center[concept] = (
            target
            if prev is None
            else ((1.0 - self.feasible_pull) * prev + self.feasible_pull * target).astype(np.float32)
        )
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        object_state = self.phase_object_state.get(concept)
        discrim_state = self.phase_discriminative_state.get(concept)
        if feasible is None or object_state is None or discrim_state is None:
            return penalty
        feasible_cost = c7.sq_dist(x, feasible)
        manifold_cost = 0.5 * c7.sq_dist(object_state, feasible) + 0.5 * c7.sq_dist(discrim_state, feasible)
        return float(penalty + self.feasible_penalty_weight * (0.55 * feasible_cost + 0.45 * manifold_cost))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    allowed.update(inspect.signature(FeasibleManifoldGeometryGrounder.__init__).parameters)
    ...
    model = FeasibleManifoldGeometryGrounder(**model_config)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C57 feasible-manifold geometry search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c57_feasible_manifold_geometry_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    stage_c56 = json.loads(
        (TEMP_DIR / "stage_c56_phase_transition_meta_switching_search_20260312.json").read_text(encoding="utf-8")
    )

    base = dict(stage_c56["best_objective_candidate"]["config"])
    ...
    for feasible_pull in [0.08, 0.18]:
        for feasible_object_weight in [0.25, 0.45]:
            for feasible_discriminative_weight in [0.25, 0.45]:
                for feasible_transition_weight in [0.20, 0.40]:
                    for feasible_memory_weight in [0.20, 0.40]:
                        for feasible_penalty_weight in [0.02, 0.05]:
                            config = {
                                **base,
                                "feasible_pull": feasible_pull,
                                "feasible_object_weight": feasible_object_weight,
                                "feasible_discriminative_weight": feasible_discriminative_weight,
                                "feasible_transition_weight": feasible_transition_weight,
                                "feasible_memory_weight": feasible_memory_weight,
                                "feasible_penalty_weight": feasible_penalty_weight,
                            }
                            rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c57_feasible_manifold_not_enough"
        core_answer = "A feasible-manifold geometry sharpens admissibility structure, but still does not produce a better retention-compatible family."
        main_open_gap = "feasible_manifold_geometry_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c57_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c57_retention_compatible_feasible_manifold_found"
        )
        core_answer = (
            "A feasible-manifold geometry creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A feasible-manifold geometry reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c57"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC57_feasible_manifold_geometry_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c57_beats_c42_retention_compatible": bool(
            ...
            "H2_c57_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
