...
class ObjectPersistenceGrounder(c7.ConsensusDiscriminatorTemporalGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        sequence_consensus_weight: float,
    ) -> None:
        super().__init__(
        ...
        self.sequence_consensus_weight = sequence_consensus_weight
        self.persistence_memory: Dict[str, np.ndarray] = {}
        self.sequence_memory: Dict[str, np.ndarray] = {}

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        concept_proto = self.full_concept_proto.get(concept)
        if concept_proto is None:
            return
        prev_persist = self.persistence_memory.get(concept)
        if prev_persist is None:
            self.persistence_memory[concept] = concept_proto.copy()
        else:
            self.persistence_memory[concept] = (
                (1.0 - self.persistence_pull) * prev_persist + self.persistence_pull * concept_proto
            ).astype(np.float32)
        canonical = self.canonical_concept.get(concept)
        if canonical is not None:
            prev_seq = self.sequence_memory.get(concept)
            target = np.concatenate([concept_proto, canonical], axis=0).astype(np.float32)
            if prev_seq is None:
                self.sequence_memory[concept] = target
            else:
                self.sequence_memory[concept] = (
                    (1.0 - self.persistence_pull) * prev_seq + self.persistence_pull * target
                ).astype(np.float32)
    ...
    def _concept_score(self, x: np.ndarray, observed: List[Tuple[str, np.ndarray]], family: str, concept: str, family_score: float) -> float:
        ...
        persist = self.persistence_memory.get(concept)
        if persist is not None:
            score += self.persistence_weight * c7.sq_dist(x, persist)
        seq = self.sequence_memory.get(concept)
        canonical = self.canonical_concept.get(concept)
        if seq is not None and canonical is not None:
            full_vec = np.concatenate([x, np.tile(canonical, 3)], axis=0).astype(np.float32)
            seq_target = np.concatenate([self.full_concept_proto[concept], np.tile(canonical, 3)], axis=0).astype(np.float32)
            score += self.sequence_consensus_weight * c7.sq_dist(full_vec, seq_target)
        return float(score)
...
def run_system(
    ...
    model = ObjectPersistenceGrounder(
        direct_weight=direct_weight,
        modality_weight=modality_weight,
        canonical_weight=canonical_weight,
        trace_weight=trace_weight,
        ...
        sequence_consensus_weight=sequence_consensus_weight,
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C10 object persistence sequence consensus search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c10_object_persistence_sequence_consensus_search_20260311.json",
    )
    args = ap.parse_args()
    ...
    baseline_consistency_ceiling = float(stage_c8["headline_metrics"]["baseline_consistency_ceiling"])

    best_objective = None
    best_consistency = None
    best_retention_compatible = None

    for persistence_weight in [0.0, 0.05, 0.1]:
        for persistence_pull in [0.0, 0.1, 0.2]:
            for sequence_consensus_weight in [0.0, 0.02, 0.05]:
                rows = []
    ...
    if best_retention_compatible is None:
        status = "stage_c10_new_mechanism_not_enough_for_strong_compatible"
        core_answer = (
            "Object persistence and sequence-level consensus improve the direct family, but do not yet produce a stronger retention-compatible closure."
        )
        main_open_gap = "needs_stronger_mechanism_block_beyond_c10"
        ...
        status = (
            "stage_c10_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c10_retention_compatible_family_strengthened"
        )
        core_answer = (
            "Object persistence and sequence-level consensus strengthen the retention-compatible direct family, but still do not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A new mechanism block reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_new_mechanism"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC10_object_persistence_sequence_consensus_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_new_mechanism_improves_retention_compatible_consistency": bool(
            ...
            "H2_new_mechanism_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
