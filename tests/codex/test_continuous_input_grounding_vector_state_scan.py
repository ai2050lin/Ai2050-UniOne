...
"""
Task block D:
vector-state dependent consolidation scan.

This is the next negative-control after scalar phase state:
- build a small vector state z_t
- let z_t encode novelty / retention pressure / family drift / confidence
- let write-back alpha depend on z_t

The question is whether simply increasing state dimensionality is enough before
we introduce a richer learned controller.
"""
...
class VectorStateConsolidator:
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        family_members = [name for name in self.phase1_concepts if proto.concept_family(name) == family]
        if family_members:
            family_mean = np.mean(np.stack([self.phase1_proto[name] for name in family_members], axis=0), axis=0)
        else:
            family_mean = prior
        family_drift = float(proto.sq_dist(family_mean, self.family_basis[family]))
        confidence = 1.0 / (1.0 + novelty)
        retention_pressure = float(np.mean([np.mean(self.stable_var[name]) for name in family_members] or [0.0]))
        target = np.array([novelty, retention_pressure, family_drift, confidence], dtype=np.float32)
        self.z = (1.0 - self.state_alpha) * self.z + self.state_alpha * target
    ...
    def state_writeback(self) -> None:
        if not self.phase2_post:
            return
        novelty, retention_pressure, family_drift, confidence = [float(value) for value in self.z]

        for concept in self.phase1_concepts:
            prev = self.unified_proto.get(concept)
            target = self.phase1_proto[concept]
            alpha = self.base_alpha * (0.7 + 0.6 * min(1.0, retention_pressure * 5.0) + 0.3 * max(0.0, 1.0 - confidence))
        ...
        for concept, posterior in self.phase2_post.items():
            ...
            family_hist = np.array(self.family_novelty[family] or [0.0], dtype=np.float32)
            post_novelty = float(min([proto.sq_dist(posterior, self.phase1_proto[name]) for name in self.phase1_concepts if proto.concept_family(name) == family] or [0.0]))
            gate_open = bool(post_novelty > np.quantile(family_hist, min(0.95, self.gate_quantile)))
            if not gate_open:
                continue
            alpha = self.base_alpha * (0.4 + 0.8 * min(2.0, novelty / (post_novelty + 1e-6)) + 0.4 * family_drift)
            alpha = float(np.clip(alpha, 0.02, 0.22))
            prev = self.unified_proto.get(concept)
    ...
    def predict(self, x: np.ndarray) -> Tuple[str, str]:
        ...
        for concept in self.phase1_concepts:
            ...
            score = 0.7 * raw_score + 0.3 * resid_score
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Vector-state consolidation scan for continuous grounding")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/continuous_input_grounding_vector_state_scan_20260309.json",
    )
    args = ap.parse_args()
    ...
    for kappa0 in [8.0, 10.0, 12.0]:
        for decay in [0.25, 0.50, 0.75]:
            for gate_quantile in [0.75, 0.85]:
                for state_alpha in [0.20, 0.30, 0.40]:
                    for base_alpha in [0.04, 0.08, 0.12]:
                        rows = []
