...
class ManifoldConditionedConsistencyHeadGrounder(c42.StrongCrossLawManifoldLiftGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.readout_pull = readout_pull
        self.readout_anchor_weight = readout_anchor_weight
        self.readout_family_weight = readout_family_weight
        self.readout_negative_weight = readout_negative_weight
        self.consistency_readout: Dict[str, np.ndarray] = {}

    def _readout_target(self, family: str, concept: str) -> np.ndarray | None:
        rows = []
        ...
        if manifold is not None:
            rows.append(0.40 * manifold.astype(np.float32))
        if family_state is not None:
            rows.append(0.20 * family_state.astype(np.float32))
        if vote is not None:
            rows.append(0.20 * vote.astype(np.float32))
    ...
    def _negative_energy(self, family: str, concept: str, readout: np.ndarray) -> float:
        negatives = []
        for other, other_family in self.concept_family.items():
            if other == concept or other_family != family:
                continue
            other_readout = self.consistency_readout.get(other)
            if other_readout is None:
                continue
            negatives.append(c7.sq_dist(readout, other_readout))
        if not negatives:
            return 0.0
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        target = self._readout_target(family, concept)
        if target is None:
            return
        prev = self.consistency_readout.get(concept)
        self.consistency_readout[concept] = (
            target
            if prev is None
            else ((1.0 - self.readout_pull) * prev + self.readout_pull * target).astype(np.float32)
        )
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if family is None or readout is None:
            return penalty
        family_state = self.family_manifold.get(family)
        penalty += self.readout_anchor_weight * c7.sq_dist(x, readout)
        if family_state is not None:
            penalty += self.readout_family_weight * c7.sq_dist(readout, family_state)
        penalty += self.readout_negative_weight * self._negative_energy(family, concept, readout)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = ManifoldConditionedConsistencyHeadGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C44 manifold-conditioned consistency head search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c44_manifold_conditioned_consistency_head_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    for readout_pull in [0.10, 0.22]:
        for readout_anchor_weight in [0.03, 0.08]:
            for readout_family_weight in [0.03, 0.08]:
                for readout_negative_weight in [0.03, 0.08]:
                    config = {
                        **base,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c44_manifold_conditioned_head_not_enough"
        core_answer = "A manifold-conditioned consistency head preserves geometry better, but still does not produce a better retention-compatible family."
        main_open_gap = "manifold_conditioned_head_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c44_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c44_retention_compatible_manifold_conditioned_head_found"
        )
        core_answer = (
            "A manifold-conditioned consistency head creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A manifold-conditioned consistency head reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c44"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC44_manifold_conditioned_consistency_head_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c44_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c44_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
