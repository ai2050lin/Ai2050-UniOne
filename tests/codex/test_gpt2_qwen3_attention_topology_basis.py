#!/usr/bin/env python
"""
Verify whether attention-defined dynamic topology also exhibits:
1) family-level shared bases
2) concept-specific offsets

We treat attention tensors as context-conditioned adjacency matrices and
analyze them in topology space directly.
"""
...
def run_attn(model, tok, text: str) -> Tuple[np.ndarray, float]:
    ...
    for layer_attn in out.attentions:
        arr = layer_attn[0].detach().float().cpu().numpy().astype(np.float32)  # [heads, seq, seq]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze shared bases in attention-defined topology")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/gpt2_qwen3_attention_topology_basis_20260308.json",
    )
    args = ap.parse_args()
