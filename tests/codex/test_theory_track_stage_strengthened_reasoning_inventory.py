...
def build_family_axes() -> dict[str, list[np.ndarray]]:
    ...
    for family in proto.FAMILIES:
        ...
        top = np.argsort(np.abs(cat))[-16:][::-1].tolist()
...
def context_templates(dim: int = 24) -> dict[str, np.ndarray]:
    ...
    return sparse_templates(specs, dim, 0.08)
...
def relation_templates(dim: int = 24) -> dict[str, np.ndarray]:
    ...
    return sparse_templates(specs, dim, 0.112)
...
def build_chain_anchors(
    ...
    for family in families:
        for chain_id in range(chains_per_family):
            ...
            for idx, weight in zip(dims, rng.uniform(0.018, 0.032, size=5).astype(np.float32)):
    ...
    for family in families:
        for chain_id in range(chains_per_family):
            ...
            for stage_idx, stage_name in enumerate(temporal_names):
                ...
                for idx, weight in zip(dims, rng.uniform(0.008, 0.016, size=4).astype(np.float32)):
...
def stage_transition_vectors(temporal_names: list[str], dim: int) -> dict[str, np.ndarray]:
    ...
    for idx, name in enumerate(temporal_names):
        ...
        vec[primary] = 0.04
        if idx > 0:
            vec[stage_primary[(idx - 1) % len(stage_primary)]] += 0.014
...
def synthesize_entry(
    ...
    for dim, weight in zip(recurrent_dims, rng.uniform(0.004, 0.012, size=4).astype(np.float32)):
    ...
    noise = rng.normal(scale=0.0022, size=cat.shape[0]).astype(np.float32)
    state = (
        cat
        + offset
        + chain_anchor
    ...
    return {
        "concept": f"{family}_stage_{concept_idx:03d}",
        "family": family,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Theory-track stage-strengthened reasoning inventory")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--concepts-per-family", type=int, default=180)
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/theory_track_stage_strengthened_reasoning_inventory_20260312.json",
    )
    args = ap.parse_args()
    ...
    chain_stage_latents = build_chain_stage_latents(rng, list(proto.FAMILIES), chains_per_family, temporal_names, dim)
    ...
    for family in proto.FAMILIES:
        for concept_idx in range(int(args.concepts_per_family)):
            context_name = context_names[(concept_idx + len(family) * 3) % len(context_names)]
            relation_name = relation_names[(concept_idx * 5 + len(family)) % len(relation_names)]
    ...
    for idx, entry in enumerate(entries):
        ...
        successor = [
            ...
            if e["temporal"] == successor_stage_name
        ]
        same_chain_successor.append(mean_pairwise(successor, state, 2))
        cross_same_successor = [
            e["state"]
            for e in entries
            if e["chain_id"] != entry["chain_id"] and e["family"] == entry["family"] and e["temporal"] == successor_stage_name
        ]
        cross_chain_successor_stage.append(mean_pairwise(cross_same_successor, state, 36))

    payload = {
        "meta": {
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "runtime_sec": float(time.time() - t0),
            "task_block": "TheoryTrack_stage_strengthened_reasoning_inventory",
            "seed": int(args.seed),
        },
        "headline_metrics": {
        ...
        "verdict": {
            "core_answer": "Stage-strengthened inventory is designed to stress temporal separation and successor coherence simultaneously, so it is a more direct test bed for priority 3+4 theorem survival.",
            "meaning": "If the stage theorem still fails here, then the weakness is likely structural rather than just due to inventory realism.",
        },
    }
