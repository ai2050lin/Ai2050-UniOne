...
class SharedManifoldTransportGrounder(c52.TriLevelCompatibilityGrounder):
    def __init__(
        self,
        *args,
        transport_pull: float,
        transport_identity_weight: float,
        transport_readout_weight: float,
        transport_family_weight: float,
        transport_direction_weight: float,
        transport_penalty_weight: float,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.transport_pull = transport_pull
        self.transport_identity_weight = transport_identity_weight
        self.transport_readout_weight = transport_readout_weight
        self.transport_family_weight = transport_family_weight
        self.transport_direction_weight = transport_direction_weight
        self.transport_penalty_weight = transport_penalty_weight
        self.transport_state: Dict[str, np.ndarray] = {}
    ...
    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        ...
        if manifold is not None:
            rows.append(1.0 * manifold.astype(np.float32))
        if trilevel is not None:
            rows.append(0.9 * trilevel.astype(np.float32))
        if readout is not None:
            rows.append(self.transport_readout_weight * readout.astype(np.float32))
        if family_state is not None:
            rows.append(self.transport_family_weight * family_state.astype(np.float32))
        if identity_anchor is not None:
            rows.append(self.transport_identity_weight * identity_anchor.astype(np.float32))
        if direction is not None:
            direct = normalize(direction.astype(np.float32))
            if trilevel is not None:
                rows.append(trilevel.astype(np.float32) + self.transport_direction_weight * direct)
            elif manifold is not None:
                rows.append(manifold.astype(np.float32) + self.transport_direction_weight * direct)

        if not rows:
            return

        target = np.mean(np.stack(rows, axis=0), axis=0).astype(np.float32)
        prev = self.transport_state.get(concept)
        self.transport_state[concept] = (
            target
            if prev is None
            else ((1.0 - self.transport_pull) * prev + self.transport_pull * target).astype(np.float32)
        )
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if transport is None:
            return penalty
        return float(penalty + self.transport_penalty_weight * c7.sq_dist(x, transport))
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    allowed.update(inspect.signature(SharedManifoldTransportGrounder.__init__).parameters)
    ...
    model = SharedManifoldTransportGrounder(**model_config)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C53 shared-manifold discriminative transport search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c53_shared_manifold_discriminative_transport_search_20260312.json",
    )
    args = ap.parse_args()
    ...
    stage_c52 = json.loads(
        (TEMP_DIR / "stage_c52_trilevel_compatibility_law_search_20260312.json").read_text(encoding="utf-8")
    )

    base = dict(stage_c52["best_objective_candidate"]["config"])
    ...
    for transport_pull in [0.08, 0.18]:
        for transport_identity_weight in [0.25, 0.45]:
            for transport_readout_weight in [0.15, 0.30]:
                for transport_family_weight in [0.08]:
                    for transport_direction_weight in [0.05, 0.12]:
                        for transport_penalty_weight in [0.02, 0.05]:
                            config = {
                                **base,
                                "transport_pull": transport_pull,
                                "transport_identity_weight": transport_identity_weight,
                                "transport_readout_weight": transport_readout_weight,
                                "transport_family_weight": transport_family_weight,
                                "transport_direction_weight": transport_direction_weight,
                                "transport_penalty_weight": transport_penalty_weight,
                            }
                            rows = []
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c53_shared_manifold_transport_not_enough"
        core_answer = "A shared-manifold discriminative transport law improves transport structure, but still does not produce a better retention-compatible family."
        main_open_gap = "shared_manifold_to_discriminative_transport_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c53_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c53_retention_compatible_transport_found"
        )
        core_answer = (
            "A shared-manifold discriminative transport law creates a stronger retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "A shared-manifold discriminative transport law reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c53"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC53_shared_manifold_discriminative_transport_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c53_beats_c42_retention_compatible": bool(
            ...
            "H2_c53_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
