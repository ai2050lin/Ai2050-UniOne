...
from stage447_polysemy_family_switch_protocol import (
    analyze_single_noun as analyze_switch_single_noun,
)
...
def build_cross_summary(model_result: Dict[str, object]) -> Dict[str, object]:
    ...
    if polysemy_split_rate >= 0.5 and switch_causality_rate >= 0.5:
        core_answer = "Gemma4 在这套多义词协议里已经表现出较明确的低重合切换结构。"
    elif gap < 0 and switch_causality_rate < 0.5:
        core_answer = "Gemma4 在当前协议里还没有稳定复现低重合多义切换，更像带明显默认词义偏置的弱切换系统。"
    else:
        core_answer = "Gemma4 在当前协议里出现了部分切换迹象，但还没有形成稳定的低重合多义切换结构。"
    return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gemma4 多义词切换内部机制协议")
...
def main() -> None:
    ...
    summary = {
        ...
        "experiment_id": "stage509_gemma4_polysemy_switch_protocol",
        "title": "Gemma4 多义词切换内部机制协议",
