...
import test_stage_c36_state_augmented_protection_search as c36
...
class MetaControllerExternalMemoryLawGrounder(c36.StateAugmentedProtectionGrounder):
    def __init__(
        self,
        direct_weight: float,
        ...
        self.law_temperature = law_temperature
        self.law_memory_gain = law_memory_gain
        self.law_identity_gain = law_identity_gain
        self.law_novelty_gain = law_novelty_gain
        self.external_map_pull = external_map_pull
        self.external_memory_map: Dict[str, np.ndarray] = {}
        self.rule_state: Dict[str, str] = {}

    def _law_logits(self, x: np.ndarray, concept: str) -> np.ndarray:
        pair = self.identity_path.get(concept)
        mem = self.memory_path.get(concept)
        nov = self.novelty_path.get(concept)
        risk = self.risk_accumulator.get(concept, self._write_risk(concept))
        id_conf = 0.0 if pair is None else 1.0 / max(1e-4, c7.sq_dist(x, pair))
        mem_conf = 0.0 if mem is None else 1.0 / max(1e-4, c7.sq_dist(x, mem))
        nov_conf = 0.0 if nov is None else 1.0 / max(1e-4, c7.sq_dist(x, nov))
        return np.array(
            [
                self.law_identity_gain * id_conf - 0.35 * risk,
                self.law_memory_gain * mem_conf + 0.45 * risk,
                self.law_novelty_gain * nov_conf - 0.15 * risk,
                0.55 * mem_conf + 0.35 * id_conf + 0.15 * nov_conf,
            ],
            dtype=np.float64,
        )

    def _choose_rule(self, x: np.ndarray, concept: str) -> str:
        logits = self._law_logits(x, concept) / max(1e-6, self.law_temperature)
        mode = int(np.argmax(logits))
        return ("identity", "memory", "novelty", "stabilize")[mode]

    def train(self, x: np.ndarray, family: str, concept: str) -> None:
        super().train(x, family, concept)
        rule = self._choose_rule(x, concept)
        self.rule_state[concept] = rule

        parts = []
        for state in (
            self.geometry_anchor.get(concept),
            self.protected_memory.get(concept),
            self.memory_path.get(concept),
            self.retention_state.get(concept),
        ):
            if state is not None:
                parts.append(state.astype(np.float32))
        if parts:
            target = np.mean(np.stack(parts, axis=0), axis=0).astype(np.float32)
            prev = self.external_memory_map.get(concept)
            self.external_memory_map[concept] = (
                target
                if prev is None
                else ((1.0 - self.external_map_pull) * prev + self.external_map_pull * target).astype(np.float32)
            )
        ...
        if rule == "memory":
            if identity is not None:
                self.identity_path[concept] = ((1.0 - 0.22) * identity + 0.22 * ext).astype(np.float32)
            if novelty is not None:
                self.novelty_path[concept] = ((1.0 - 0.30) * novelty + 0.30 * ext).astype(np.float32)
        elif rule == "identity":
            if identity is not None and memory is not None:
                self.identity_path[concept] = ((1.0 - 0.10) * identity + 0.10 * memory).astype(np.float32)
        elif rule == "novelty":
            if novelty is not None and memory is not None:
                self.novelty_path[concept] = ((1.0 - 0.08) * novelty + 0.08 * memory).astype(np.float32)
        else:
            if identity is not None:
                self.identity_path[concept] = ((1.0 - 0.14) * identity + 0.14 * ext).astype(np.float32)
            if novelty is not None:
                self.novelty_path[concept] = ((1.0 - 0.18) * novelty + 0.18 * ext).astype(np.float32)
    ...
    def _fusion_penalty(self, x: np.ndarray, concept: str) -> float:
        ...
        if ext is None or rule is None:
            return base
        mismatch = c7.sq_dist(x, ext)
        if rule == "memory":
            return float(base + 0.12 * mismatch)
        if rule == "identity":
            return float(base + 0.04 * mismatch)
        if rule == "novelty":
            return float(base + 0.03 * mismatch)
        return float(base + 0.08 * mismatch)
...
def run_system(seed: int, noise: float, dropout_p: float, missing_modality_p: float, config: Dict[str, float]) -> Dict[str, float]:
    ...
    model = MetaControllerExternalMemoryLawGrounder(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage C37 meta-controller / external memory law search")
    ...
    ap.add_argument(
        "--json-out",
        type=str,
        default="tests/codex_temp/stage_c37_meta_controller_external_memory_law_search_20260311.json",
    )
    args = ap.parse_args()

    t0 = time.time()
    stage_c36 = json.loads(
        (TEMP_DIR / "stage_c36_state_augmented_protection_search_20260311.json").read_text(encoding="utf-8")
    )
    stage_c15 = json.loads(
        (TEMP_DIR / "stage_c15_stronger_long_horizon_binder_search_20260311.json").read_text(encoding="utf-8")
    )
    base = dict(stage_c36["best_objective_candidate"]["config"])
    ...
    for law_temperature in [0.50, 1.00]:
        for law_memory_gain in [1.2, 1.8]:
            for law_identity_gain in [0.9, 1.3]:
                for external_map_pull in [0.08, 0.18]:
                    config = {
                        **base,
                        "law_temperature": law_temperature,
                        "law_memory_gain": law_memory_gain,
                        "law_identity_gain": law_identity_gain,
                        "law_novelty_gain": 0.8,
    ...
    if best_retention_compatible is None:
        ...
        status = "stage_c37_meta_memory_law_not_enough"
        core_answer = "Meta-controller plus externalized memory law changes the rule-selection layer, but still does not produce a new retention-compatible family."
        main_open_gap = "meta_rule_selection_still_not_enough"
    else:
        best_compatible_consistency = float(best_retention_compatible["summary"]["crossmodal_consistency"])
        status = (
            "stage_c37_strong_direct_closure_found"
            if best_compatible_consistency >= strong_multimodal_target
            else "stage_c37_retention_compatible_meta_memory_law_found"
        )
        core_answer = (
            "Meta-controller plus externalized memory law creates a new retention-compatible family, but still does not reach the strong target."
            if best_compatible_consistency < strong_multimodal_target
            else "Meta-controller plus externalized memory law reaches strong direct closure."
        )
        main_open_gap = (
            "strong_target_gap_remains_after_c37"
            if best_compatible_consistency < strong_multimodal_target
    ...
    payload = {
        "meta": {
            ...
            "task_block": "StageC37_meta_controller_external_memory_law_search",
        },
        "headline_metrics": {
        ...
        "hypotheses": {
            "H1_c37_beats_c15_retention_compatible": bool(
                best_retention_compatible is not None
                and best_compatible_consistency > float(incumbent["crossmodal_consistency"])
            ),
            "H2_c37_reaches_strong_target": bool(best_compatible_consistency >= strong_multimodal_target),
        },
        "verdict": {
            "status": status,
