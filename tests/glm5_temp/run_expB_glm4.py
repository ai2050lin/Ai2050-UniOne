"""
Phase 36 ExpB 精简版: 训练 vs 未训练模型对照
只测试1个概念、2个源层、5个trial
"""
import sys
...
CONCEPTS = ["apple"]  # 只测1个概念
SOURCE_LAYERS = [0, 20]  # 只测2个层
N_TRIALS = 5
EPS = 0.01
N_SVD = 200  # W_U SVD分量数
...
def compute_proj_ratio(delta, basis):
    proj = basis @ delta  # [k]
    proj_energy = np.sum(proj**2)
    total_energy = np.sum(delta**2)
    if total_energy < 1e-20:
        return 0.0
    return proj_energy / total_energy

def run_test(model, tokenizer, device, model_name, basis_wu, d_model, n_layers, tag=""):
    """在给定模型上运行投影比测试"""
    results = {}
...
def main():
    ...
    print(f"Phase 36 ExpB 精简版: {model_name}", flush=True)
    ...
    print(f"  W_U subspace: {N_SVD} components, energy={np.sum(S_wu[:N_SVD]**2)/np.sum(S_wu**2):.4f}", flush=True)
    
    trained_results = run_test(model, tokenizer, device, model_name, basis_wu, d_model, n_layers, tag="训练")
    ...
    # 复制训练好的lm_head
    print("  复制训练好的lm_head...", flush=True)
    ...
    # 4. 在未训练模型上测试
    untrained_results = run_test(untrained_model, tokenizer, device, model_name, basis_wu, d_model, n_layers, tag="未训练")
    ...
    for concept in CONCEPTS:
        for src_l_str in [str(sl) for sl in SOURCE_LAYERS]:
            ...
            supports_H0 = abs(tr_delta - ut_delta) < abs(tr_delta) * 0.3
            
            verdict = "H1(路由是学来的)" if supports_H1 else ("H0(架构固有)" if supports_H0 else "不确定")
