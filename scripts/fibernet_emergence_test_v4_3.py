"""
FiberNet v4.3 - 健壮版
修复: 除零错误, 增强 DataLoader 稳定性, 完善分析逻辑
"""
...
class CharDataset(Dataset):
    def __init__(self, text, seq_len=128, vocab=None):
        ...
        # 预先过滤掉不在词表中的字符
        self.data = torch.tensor([self.c2i[c] for c in text if c in self.c2i], dtype=torch.long)
...
class ScaledFiberNet(nn.Module):
    def __init__(self, vocab_size, d_model=256, n_layers=8, nhead=8, d_ff=1024, dropout=0.1, max_len=512):
        ...
        self.pos_embed = nn.Embedding(max_len, self.d_logic); self.tok_embed = nn.Embedding(vocab_size, self.d_memory)
...
class ScaledTransformer(nn.Module):
    def __init__(self, vocab_size, d_model=256, n_layers=8, nhead=8, d_ff=1024, dropout=0.1, max_len=512):
        super().__init__()
        self.tok_embed = nn.Embedding(vocab_size, d_model); self.pos_embed = nn.Embedding(max_len, d_model)
        self.drop = nn.Dropout(dropout)
        self.layers = nn.ModuleList([nn.TransformerEncoderLayer(d_model, nhead, d_ff, dropout, batch_first=True, activation='gelu') for _ in range(n_layers)])
    ...
    def forward(self, x):
        B, S = x.shape; mask = nn.Transformer.generate_square_subsequent_mask(S, device=x.device)
        pos = torch.arange(S, device=x.device).unsqueeze(0)
        h = self.drop(self.tok_embed(x) + self.pos_embed(pos))
        for layer in self.layers: h = layer(h, src_mask=mask, is_causal=False)
...
class EmergenceDetector:
    @staticmethod
    def analyze(acts):
        res = {'b1': 0, 'b1sig': 0, 'dim': 0.0, 'per': False}
        if acts is None or len(acts) < 10: return res
        try: 
            from ripser import ripser
            subs = acts[np.random.choice(len(acts), min(1500, len(acts)), False)]
            subs = (subs - subs.mean(0))/(subs.std(0)+1e-8)
            r = ripser(subs, maxdim=1, thresh=2.0)['dgms']
            if len(r)>1:
                h1 = r[1]; res['b1'] = len(h1)
                res['b1sig'] = len([p for p in h1 if (p[1]-p[0])>0.1])
        except: pass
        try:
...
def train_ep(model, ld, opt, dev, ep, name):
    ...
    for i, (x, y) in enumerate(ld):
        ...
        if (i+1)%100==0 or (i+1)==n: 
            avg_l = L / max(T, 1)
            print(f"\r[{name}] Ep {ep} | {i+1}/{n} | loss={avg_l:.4f} | {(time.time()-t0):.0f}s", end='', flush=True)
    print(); return L/max(T,1)
...
def main():
    print("Start v4.3 (Robust)...")
    DEV = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    txt = download_shakespeare(); vc = sorted(set(txt)); vocab = {c: i for i, c in enumerate(vc)}
    sp = int(len(txt)*0.9); ds_t = CharDataset(txt[:sp], vocab=vocab); ds_v = CharDataset(txt[sp:], vocab=vocab)
    ld_t = DataLoader(ds_t, 64, shuffle=True, drop_last=True); ld_v = DataLoader(ds_v, 64, drop_last=True)
    VS = len(vocab); fnet = ScaledFiberNet(VS).to(DEV); tnet = ScaledTransformer(VS).to(DEV)
    results = {}
    for n, m in [("FiberNet", fnet), ("Transformer", tnet)]:
        print(f"\nTraining {n}..."); opt = torch.optim.AdamW(m.parameters(), lr=3e-4); hist = []
        ...
        torch.save(m.state_dict(), os.path.join(DATA_DIR, f"{n}_v4_3.pth"))
        acts = get_acts(m, ld_v, DEV); emg = EmergenceDetector.analyze(acts)
        results[n] = {'history': hist, 'emergence': emg}
    json.dump(results, open(os.path.join(DATA_DIR, "v4_3_report.json"), "w"))
    print("\nDone. Report saved.")
