...
class CharDataset(Dataset):
    def __init__(self, text, seq_len=128, vocab=None):
        ...
        if vocab is None:
            ...
            self.c2i = vocab
            self.i2c = {i: c for c, i in vocab.items()}
...
def main():
    print("Start v4.2 (Fixed Vocab)...")
    DEV = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    txt = download_shakespeare()
    # 全局词表 !
    vocab_chars = sorted(set(txt))
    vocab = {c: i for i, c in enumerate(vocab_chars)}
    
    sp = int(len(txt)*0.9)
    ds_t = CharDataset(txt[:sp], vocab=vocab)
    ds_v = CharDataset(txt[sp:], vocab=vocab)
    ...
    for n, m in [("FiberNet", fnet), ("Transformer", tnet)]:
        ...
        torch.save(m.state_dict(), os.path.join(DATA_DIR, f"{n}_v4_2.pth"))
        print(f"Analyzing {n}...")
    ...
    json.dump(results, open(os.path.join(DATA_DIR, "v4_2_report.json"), "w"))
    print("\nDone. Report saved.")
