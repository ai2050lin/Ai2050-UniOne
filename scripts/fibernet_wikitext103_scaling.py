...
class EmergenceMonitor:
    ...
    def calc_id(acts):
        ...
        try:
            ...
            d = d[:, 1:] # Drop self
            mu = d[:, 1] / d[:, 0]
            est = -1 / np.log(mu) # Hill estimator simplified
            # Trim extremes
...
def train():
    if not os.path.exists(os.path.join(DATA_DIR, "train.pt")):
        print("Data not found. Please run preprocessing first.")
        return

    train_ds = WikiTextDataset(os.path.join(DATA_DIR, "train.pt"), SEQ_LEN)
    ...
    print(f"[*] Starting WikiText-103 Scaling (100M Params, 20 Epochs)")
    ...
    for ep in range(1, EPOCHS+1):
        ...
        print(f"[*] Analyzing Geometry (Ep {ep})...")
        model.eval()
        with torch.no_grad():
            # Sample a few batches for ID
            acts = []
            for j, (x, _) in enumerate(train_dl):
                if j > 10: break
                _, _, mem = model(x.to(DEV))
                acts.append(mem.cpu().numpy())
            
            acts = np.concatenate(acts, axis=0) # [B*10, T, D]
            id_val = EmergenceMonitor.calc_id(acts)
            print(f"    >> ID: {id_val:.2f} | Loss: {avg_loss:.4f}")
            
            results.append({"ep": ep, "loss": avg_loss, "id": id_val})
            with open(os.path.join(RES_DIR, "phase4_wiki103_results.json"), "w") as f:
                json.dump(results, f)
