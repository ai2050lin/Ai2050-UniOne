...
import samplePayload from './data/local_pulse_region_parameter_family_learner_sample.json';
...
const SYSTEM_LABELS = {
  ...
  manual_recovery_aware: '手工恢复参数族',
  learned_region_family: '学习得到的参数族',
};

export default function LocalPulseRegionParameterFamilyLearnerDashboard() {
  ...
  async function onUpload(event) {
    ...
    try {
      ...
      setError(`脑区参数族学习器 JSON 导入失败: ${err?.message || '未知错误'}`);
    }
  }
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '16px',
        background:
          'radial-gradient(circle at top left, rgba(16,185,129,0.16), transparent 28%), radial-gradient(circle at bottom right, rgba(59,130,246,0.12), transparent 26%), rgba(2,6,23,0.68)',
        border: '1px solid rgba(16,185,129,0.24)',
      }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#f8fafc', fontSize: '15px', fontWeight: 'bold' }}>五点四十六、脑区参数族学习器</div>
          <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginTop: '4px', maxWidth: '860px' }}>
            这组基准不再手工指定一条区域化参数族，而是在 `recovery_aware` 附近自动搜索。核心问题是：统一局部编码机制能不能自己学出一个比手工参数族更好的三阶段闭环候选。
          </div>
        </div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '12px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>1. 手工族与学习族对照</div>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={systemRows} margin={{ top: 10, right: 12, left: 0, bottom: 16 }}>
              ...
              <Bar dataKey="参数族离散度" fill="#f59e0b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>2. 顶部搜索候选</div>
          <ResponsiveContainer width="100%" height={320}>
