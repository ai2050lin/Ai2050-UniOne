...
import samplePayload from './data/local_pulse_region_family_generator_sample.json';
...
const SYSTEM_LABELS = {
  ...
  learned_region_family: '学习得到的参数族',
  generated_region_family: '5维生成器参数族',
};

export default function LocalPulseRegionFamilyGeneratorDashboard() {
  ...
  async function onUpload(event) {
    ...
    try {
      ...
      setError(`脑区参数族生成器 JSON 导入失败: ${err?.message || '未知错误'}`);
    }
  }
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '16px',
        background:
          'radial-gradient(circle at top left, rgba(6,182,212,0.16), transparent 28%), radial-gradient(circle at bottom right, rgba(16,185,129,0.12), transparent 26%), rgba(2,6,23,0.68)',
        border: '1px solid rgba(6,182,212,0.24)',
      }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#f8fafc', fontSize: '15px', fontWeight: 'bold' }}>五点四十七、脑区参数族生成器</div>
          <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginTop: '4px', maxWidth: '860px' }}>
            这组基准继续把脑区参数族压缩成低维生成器。核心问题是：5 维左右的生成坐标，能否复现甚至超过当前学习得到的参数族，而不是继续暴力搜索整套参数。
          </div>
        </div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '10px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>生成器闭环增益</div>
          <div style={{ color: '#22c55e', fontSize: '16px', fontWeight: 'bold', marginTop: '4px' }}>
            {fmt(payload?.headline_metrics?.generated_three_stage_gain)}
          </div>
        </div>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>闭环平衡变化</div>
          <div style={{ color: '#a855f7', fontSize: '16px', fontWeight: 'bold', marginTop: '4px' }}>
            {fmt(payload?.headline_metrics?.generated_balance_gain)}
          </div>
        </div>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>生成器维度</div>
          <div style={{ color: '#06b6d4', fontSize: '16px', fontWeight: 'bold', marginTop: '4px' }}>
            {payload?.headline_metrics?.generator_dim ?? '-'}
          </div>
        </div>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>手工控制维估计</div>
          <div style={{ color: '#f59e0b', fontSize: '16px', fontWeight: 'bold', marginTop: '4px' }}>
            {payload?.headline_metrics?.manual_control_dim_estimate ?? '-'}
          </div>
        </div>
      </div>
        ...
        {Object.entries(payload?.hypotheses || {}).map(([key, value]) => (
          <div
            ...
            style={{
              ...
              padding: '6px 10px',
              fontSize: '11px',
              color: value ? '#cffafe' : '#fee2e2',
              background: value ? 'rgba(6,182,212,0.18)' : 'rgba(248,113,113,0.18)',
              border: `1px solid ${value ? 'rgba(6,182,212,0.30)' : 'rgba(248,113,113,0.30)'}`,
            }}
          >
            {`${key}: ${value ? '成立' : '未成立'}`}
          </div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '12px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>1. 低维生成器与手工/学习族对照</div>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={systemRows} margin={{ top: 10, right: 12, left: 0, bottom: 16 }}>
              ...
              <Bar dataKey="参数族离散度" fill="#06b6d4" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>2. 顶部生成候选</div>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={trialRows} margin={{ top: 10, right: 12, left: 0, bottom: 16 }}>
              <CartesianGrid stroke="rgba(148,163,184,0.18)" strokeDasharray="3 3" />
