...
import samplePayload from './data/relation_protocol_head_causal_sample.json';
...
export default function RelationProtocolHeadCausalDashboard() {
  ...
  const relationBars = useMemo(() => {
    ...
    return Object.entries(rows).map(([relation, value]) => ({
      relation,
      top_collapse_ratio: Number(value?.summary?.top_collapse_ratio || 0),
      control_collapse_ratio: Number(value?.summary?.control_collapse_ratio || 0),
      causal_margin: Number(value?.summary?.causal_margin || 0),
      baseline_peak_bridge_tt: Number(value?.summary?.baseline_peak_bridge_tt || 0),
    }));
  }, [modelRow]);
  ...
  async function onUpload(event) {
    ...
    try {
      ...
      setError(`关系协议因果 JSON 导入失败: ${err?.message || '未知错误'}`);
    }
  }
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '16px',
        background:
          'radial-gradient(circle at top left, rgba(248,113,113,0.12), transparent 28%), radial-gradient(circle at top right, rgba(56,189,248,0.1), transparent 24%), rgba(2,6,23,0.64)',
        border: '1px solid rgba(248,113,113,0.24)',
      }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#e2e8f0', fontSize: '15px', fontWeight: 'bold' }}>关系协议头因果验证</div>
          <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginTop: '4px' }}>
            比较最佳头消融与同层对照头消融，判断头级 atlas 找到的是“因果承载头”还是“相关候选头”。
          </div>
        </div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>平均最佳头塌缩率</div>
          <div style={{ color: '#fca5a5', fontSize: '20px', fontWeight: 'bold' }}>{pct(globalSummary.mean_top_collapse_ratio)}</div>
        </div>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>平均对照塌缩率</div>
          <div style={{ color: '#7dd3fc', fontSize: '20px', fontWeight: 'bold' }}>{pct(globalSummary.mean_control_collapse_ratio)}</div>
        </div>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
      ...
      <div style={{ marginTop: '14px', border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
        <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>1. 各关系族最佳头 vs 对照头塌缩率</div>
        <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginBottom: '8px' }}>
          若红柱系统性高于蓝柱，说明最佳头具有更强因果性；若两者接近，说明协议更可能是冗余分布式实现。
        </div>
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={relationBars} margin={{ top: 10, right: 12, left: 0, bottom: 10 }}>
            ...
            <Bar dataKey="top_collapse_ratio" name="最佳头塌缩率" fill="#f87171" radius={[4, 4, 0, 0]} />
            <Bar dataKey="control_collapse_ratio" name="对照头塌缩率" fill="#38bdf8" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
