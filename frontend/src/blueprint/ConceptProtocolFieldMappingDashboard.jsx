...
import samplePayload from './data/concept_protocol_field_mapping_sample.json';
...
export default function ConceptProtocolFieldMappingDashboard() {
  ...
  async function onUpload(event) {
    ...
    try {
      ...
      setError(`概念到协议场映射 JSON 导入失败: ${err?.message || '未知错误'}`);
    }
  }
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#e2e8f0', fontSize: '15px', fontWeight: 'bold' }}>概念到协议场调用映射</div>
          <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginTop: '4px' }}>
            展示 `U(c, tau, l, h)` 的可视化摘要：概念最偏好的协议场、主要调用层群、头部质量分布，以及调用集中度是偏稀疏还是偏分布式。
          </div>
        </div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px' }}>
        ...
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '12px', padding: '10px' }}>
          <div style={{ color: '#94a3b8', fontSize: '11px' }}>首选场是否匹配真值</div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(340px, 0.95fr)', gap: '12px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          ...
          <div style={{ color: '#94a3b8', fontSize: '11px', lineHeight: '1.7', marginBottom: '8px' }}>
            不是只看这个概念像谁，而是看它在协议提示下，真正把哪一个协议场调起来。
          </div>
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
            ...
            <div style={{ color: '#cbd5e1', fontSize: '12px', lineHeight: '1.8' }}>
              ...
              <div>{`真值协议场: ${conceptRow?.true_field || '-'}`}</div>
