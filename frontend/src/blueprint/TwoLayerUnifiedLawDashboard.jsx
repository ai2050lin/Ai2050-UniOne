...
import samplePayload from './data/two_layer_unified_law_sample.json';

function isValidPayload(payload) {
  return Boolean(payload && payload.meta && payload.ranking_layer && payload.two_layer_law && payload.leave_one_out);
}
...
export default function TwoLayerUnifiedLawDashboard() {
  ...
  const compareRows = useMemo(
    () => [
      ...
      {
        law: '双层统一律',
        mean_absolute_gap: Number(payload?.two_layer_law?.mean_absolute_gap || 0),
        score_correlation: Number(payload?.two_layer_law?.score_correlation || 0),
      },
    ],
    [payload]
  );
  ...
  async function onUpload(event) {
    ...
    try {
      ...
      if (!isValidPayload(parsed)) throw new Error('缺少 ranking_layer、two_layer_law 或 leave_one_out 字段');
      setPayload(parsed);
      setSource(`文件导入: ${file.name}`);
      setError('');
    } catch (err) {
      setError(`双层统一律 JSON 导入失败: ${err?.message || '未知错误'}`);
    }
  }
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '16px',
        background:
          'radial-gradient(circle at top left, rgba(168,85,247,0.12), transparent 28%), radial-gradient(circle at top right, rgba(56,189,248,0.08), transparent 24%), rgba(2,6,23,0.64)',
        border: '1px solid rgba(168,85,247,0.22)',
      }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#e2e8f0', fontSize: '15px', fontWeight: 'bold' }}>双层统一律</div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '10px' }}>
        ...
        <MetricCard label="双层统一律误差" value={fmt(payload?.two_layer_law?.mean_absolute_gap)} />
        <MetricCard label="双层统一律相关系数" value={fmt(payload?.two_layer_law?.score_correlation)} />
        <MetricCard label="误差改善" value={fmt(payload?.two_layer_law?.gap_improvement_vs_ranking)} />
        <MetricCard label="相关改善" value={fmt(payload?.two_layer_law?.correlation_improvement_vs_ranking)} />
        <MetricCard label="留一法平均误差" value={fmt(payload?.leave_one_out?.mean_held_out_gap)} />
        <MetricCard label="留一法状态" value={payload?.leave_one_out?.pass ? '通过' : '未通过'} />
      </div>

      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '12px' }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '10px' }}>
          <div style={{ color: '#dbeafe', fontSize: '12px', marginBottom: '6px' }}>1. 排序层 vs 双层统一律</div>
      ...
      <div style={{ marginTop: '14px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '12px' }}>
        ...
        <div style={{ border: '1px solid rgba(148,163,184,0.22)', borderRadius: '14px', padding: '12px' }}>
          ...
          <div style={{ color: '#cbd5e1', fontSize: '12px', lineHeight: '1.8' }}>
            双层统一律在当前样本上显著优于单一排序层，但解析式标定层在外推上仍不稳定。这个结果说明“排序层 + 标定层”方向是对的，
            下一步应把标定层从闭式拟合推进到可学习版本，再判断统一律能否真正进入可训练阶段。
          </div>
